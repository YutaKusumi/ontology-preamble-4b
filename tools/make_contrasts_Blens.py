...
T = {
    ...
    'nulls': {
        ...
        'B_random': {'count': TB['random_control']['count'], 'main_seed': TB['seeds']['random_dirs']['main'], 'tune_seed': TB['seeds']['random_dirs']['tune'],
                     'rule': '`steer_B.random_directions` を凍結のまま呼んで再生する（B の本走行と調整走行で使った方向）',
                     'repro_check': '写した器が、凍結の関数の再生（本走行と調整走行の種で、層ごとに引く全ての方向）とビットで一致することを確かめる。手元で再生した方向の SHA-256 を凍結の記録に置き、Colab の起動器が B の版の NumPy で再生した値と突き合わせ、合わなければ層一の計算に進まない'},
    ...
    'magnitude': {
        ...
        'calibration_check': {'ci': 0.999, 'rule': '無操作の腕の、変換の後の確率と、無操作の観測の率を突き合わせる——主位置では、選び方の層の升目ごとの様式の率（その升目の無操作の試行の全件が分母）、答えの文字の位置では、JSON 直答の出力の中の文字の率（JSON 直答の出力が分母）。観測の件数が、その確率の二項分布の中央の区間（`ci`）の外なら、止めて登録者に相談する'},
