...
v4（2026-09-24・草案3 の確かめの後）: 登録者裁定 D186（草案3 の確かめ）と D187（凍結の前の Colab の確かめで、段階 B のランダム方向の再生が機械の違いでビットで一致しなかったことを受けて、再生の確かめを方向ごとの相対の差の許容に改め、torch を CUDA の組みまで B の版に揃える）を反映。版の名（`version`）は草案3 のまま置く（予想の書式 v1 の「正本の版」の欄と合わせるため・予想の項目は変えていない）。
...
T = {
    ...
    'numbering': {'rulings_next': 'D188', 'verification_next': 'K361', 'adoption_next': 'P565'},
