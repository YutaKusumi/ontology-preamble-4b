...
measured = {
    ...
    's4_seal_right': {'now_at_effect': 0.122, 'equivalence_at_effect': 0.053,
                      'now_at_zero': 0.868, 'equivalence_at_zero': 0.817,
                      'partner_base': 0.1725,
                      'note': '真の低下が登録した効き目ちょうどのとき／真に何も起きていないときに「封印は当たり」と書く割合'},
...
RANDOM = {'count': 3, 'norm_matched': True, 'per_layer': True,
          ...
          'homogeneity_rule': ('**等質性が崩れたときの札を置く**（裁定 D127・2026-09-18）: 三方向の率の最大と最小の差が %d pt を超えたら、その対比に**注（ランダム方向の不均一）**を付け、帰無が「ランダム方向一般」ではなく「引いた三本」であることを報告に印字する。\n前は「記述で確かめる」だけで、崩れたときの帰結が無かった（系統内外の票・採否表 P355）') % 15,
...
quality = {'items': 200, 'threshold_pt': -10, 'numerator': '正答数', 'denominator': 200,
           'denominator_name': ('**`denominator` は「登録した問いの数」であって率の分母ではない**（裁定 D127・2026-09-18）。率の分母は `denominator_rule` のとおり「使えた試行」である。\n`format_fail_rule` の「分母 %d を保つ」は**書式外を不正解に数える**という意味であり、api_error を分母に残す意味ではない。前はこの二つが正本の中で二通りに読めた（系統外の検分・採否表 P366）') % 200,
           'api_error_gate': ('**api_error の率が腕の間で %d pt を超えて違うセルは、品質床を判定しない**（裁定 D127・2026-09-18）。介入が例外を起こすとその試行は分母から消えるので、差のある欠測を門で止める') % 10,
...
reading_D128 = {
    'nk_normalised': ('**Nk の効果は「人工的に規格化した強さ」でのものである**（裁定 D128・2026-09-18）。裁定 D102 で全方向を ‖v̂〔static〕‖ に合わせたので、Nk 方向（前置きの短い腕対の差）は**本来の表現の強さではなく、O の枠組みの差の大きさに引き伸ばされて**注がれる。交差族が有意でも「Nk 方向が効いた」とは書かず、「規格化した強さの Nk 方向が効いた」と書く（系統外の検分・採否表 P344）'),
...
T = {'id': 'contrasts-B', 'version': 'draft11-2026-09-18',
     ...
     'decisions': {'D4a': '主抽出位置・選定の一段化・門1・調整走行の腕あたりの n=%d（2026-09-13 承認）' % n_tune, 'D5': '実在する腕対の差方向の統制（2026-09-13 承認）',
                   ...
                   'D100': '直した器材は同じ二体に見せる（甲・2026-09-18 承認・**登録者が「同じ個体は呼べない／同じ票を持たせた新規個体で」と正した**）',
                   'D101': '段階 B は chat template を当てる。帯の起点は組み立て済みの列の中で場面本文が始まる位置（甲・2026-09-18 承認）',
                   ...
                   'D118': 'S4 の「下がらなかった」を同等性で決める（甲・2026-09-18 承認・効き目ちょうどでの誤りが下がる・値は `measured.s4_seal_right`）',
     ...
     'measured': measured, 'reading_D128': reading_D128, 'disclosure': disclosure, 'selftest_rule': selftest_rule, 'print_strings': print_strings, 'denominators': denominators, 'publication': publication,
