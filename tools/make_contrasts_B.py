...
LAYERS = ['1/4', '1/2', '3/4']       # 全層に対する深さ（凍結時に層番号へ翻訳して記帳）
...
DIRS = {'6a': {'def': 'h_O − h_Osec（静的・抽出場面の平均）', 'role': '確証族の v̂'},
        '6b': {'def': 'h_{O-Ncold} − h_{Osec-Ncold}（負荷下・抽出場面の平均）', 'role': 'S4 の反証（記述）'},
        'Nk': {'def': 'h_Nk − h_N（問い 3 の交差族）', 'role': '交差族の v̂'},
        'td': {'def': 'h_Onull − h_N（実在する腕対の差方向・ノルムを v̂ に合わせる）', 'role': '記述の統制（裁定 D5）'}}
RANDOM = {'count': 3, 'norm_matched': True, 'per_layer': True, 'pooling': '3 本の試行を合併して一腕 v_random とする（合併の前に 3 方向の率を印字し、二項の等質性を記述で確かめる）', 'seed': 71001}
...
sub = [ct('sub', sc, 'O-Ncold-v', 'O-Ncold-vrand', base_arm='O-Ncold', base_4B2507=base_of(sc, 'O-Ncold').get('k'), base_n=base_of(sc, 'O-Ncold').get('n'), direction_v='6a') for sc in SC]
add = [ct('add', sc, 'Onull+v', 'Onull+vrand', base_arm='Onull', base_4B2507=base_of(sc, 'Onull').get('k'), base_n=base_of(sc, 'Onull').get('n'), direction_v='6a') for sc in SC]
...
desc = {
    ...
    'B_desc_S4': {'question': 'S4 の反証（記述・族の外）: (6b) を S4 の Osec-Ncold に加算しても下がらない、または上がる、と凍結時に封印する。Osec-Ncold が S4 で床または天井にあれば、同じ余地の条項で測れない',
                  'contrasts': [ct('d', FALSIFY_SC, 'Osec-Ncold+v6b', 'Osec-Ncold+vrand', direction_v='6b')], 'sealed_prediction': 'to_be_sealed_at_freeze',
                  'base_4B2507': base_of(FALSIFY_SC, 'Osec-Ncold').get('k'), 'base_n': base_of(FALSIFY_SC, 'Osec-Ncold').get('n')},
    'B_desc_layer': {'question': '問い 5: O-Ncold と Osec-Ncold の表現がどの層から分かれるか（層別射影差・記述・裁定 D4 (a) で試行単位の p を印字しない）'},
    'B_desc_direction': {'question': '問い 1・6: 方向の有無と二系統（(6a)・(6b)）の比較（コサイン・ノルム・層ごとの分離・記述）。主抽出位置の活性は腕 × 場面で決まるため、試行単位の検定を置かない'},
    'B_desc_dose': {'question': '問い 7: O の文単位の削除・入替による用量反応と、差を作った文の表現差（記述）'},
...
selection = {
    ...
    'candidates': {'layers': LAYERS, 'coefficients': COEFS, 'count': len(LAYERS) * len(COEFS), 'note': '位置は主に固定し、層 × 係数の一段で選ぶ（裁定 D4 (a)・第 1 段の保留 AUC は廃止）'},
    ...
    'equivalence_band': '最大の候補との差の 95% 区間（二項の差・正規近似）が 0 を含む候補を同値とする',
...
quality = {'items': 200, 'threshold_pt': -10, 'numerator': '正答数', 'denominator': 200,
           ...
           'partner': '同じ腕の無操作（同じ 200 問・二標本で比べる）', 'unit': '腕 × 層 × 係数',
           'cells': len(LAYERS) * len(COEFS) * 2, 'source': '公開の日本語能力課題の固定断片（出所・版・ライセンス・断片の SHA を凍結時に記帳・候補は草案5B §5 で諮る）',
...
style_gate = {'hold_pt': 30, 'note_pt': 15, 'strict': True, 'unit': '対比 × 場面', 'numerator': '該当試行', 'denominator': 'n_ok',
              ...
              'asymmetry': '様式門は確証の札にのみ作用する（非有意の対比に 30 pt 超の様式差があっても保留も注も付かない）。この非対称を報告の族ごとの結論に書く'}
identity = {'gate': '0.5（段階 A と共用）', 'stacks': ['API', 'vLLM', 'transformers'], 'n': n_id, 'scenario': 'N1', 'arms': 13,
            'metric': '段階 A §2.9 と同じ（各セルの絶対差の平均 ≤5 pt かつ最大 ≤12 pt）',
...
runner = {'batch': 16, 'batch_rule': 'バッチ生成 16 を設計定数にする（裁定 D3 (d)・調整走行の最初のセッションで実測し、転記行を置き換える）',
...
report_rules = {'typed_numbers': '報告に打ち込む数は、日付・SHA16・SHA-256・費用の実額・コミットの短い名に限る。ほかの数はすべて機械の区画から出す',
                'kanji_counts': '起草者の文の漢数字の件数（一本・三対比など）の扱いは未決（逸脱 D-45・草案5B §5 で決める——走査器が拾うか、正本の文言に扱いを書くか）',
...
carryover_A = {'status': '草案5B §5 で範囲を諮る（裁定 D61 以降）', 'source': '反映メモ A §3（第一巡の採否表 §6・見直しの B 類・最終検分の条件 B1・起草者の足し）',
...
procedure = ['同一性選別（三スタック・段階 A の門0.5 と共用・transformers 経路 13 腕 × n=160 × N1）',
             ...
             '報告草案 → 検分 → 公開 → 反映メモ B']
print_strings = {
    'first_finding': '確証の族 16 対比のうち、確証 {confirmed}・判定不能（検閲） {undecidable}・判定保留（refuse 転位） {refuse}・判定保留（様式転位） {style}・非有意 {ns}。',
    'label_confirmed': '{A} は {B}（ランダム方向）と異なった（向き {sign}・pt 差 {diff} pt・区間 {ci}）。無操作との差は記述として併置する。',
    'gate1_closed': '門1: 品質床に合格する層 × 係数が 0。操作不能を記帳し、B′ へ進まない（方向の非存在は書かない）。',
...
T = {'id': 'contrasts-B', 'version': 'draft5-2026-09-18',
     ...
     'decisions': {'D4a': '主抽出位置・選定の一段化・門1・調整走行 n=100（2026-09-13 承認）', 'D5': '実在する腕対の差方向の統制（2026-09-13 承認）',
                   'D3d': 'バッチ生成 16（2026-09-13 承認）', 'D7': '同一性選別の手元 n=160（2026-09-13 承認）',
