...
quality = {'items': 200, 'threshold_pt': -10, 'numerator': '正答数', 'denominator': 200,
           'denominator_rule': '**差は、腕と相手のそれぞれで「使えた試行」を分母にした正答率の差（pt）として取る**（裁定 D104・2026-09-18）。'
                               ...
                               '（直しの確認の巡で実際に反転した・採否表 P305）。上の `denominator` は**登録した問いの数**であって、率の分母ではない' % 200,
...
gate_order = {'gate_stop': GATE_STOP, 'order': ['判定不能（採点欠落）', '検閲（両腕条件）', '希釈の門（書式外の差）', '希釈の門（refuse の差）', 'refuse 門（答えた分母）', '様式門', '品質床（選定後）'],
...
runner = {'batch': 16, 'batch_rule': 'バッチ生成 %d を設計定数にする（裁定 D3 (d)・調整走行の最初のセッションで実測し、転記行を置き換える）' % 16,
          ...
          'prompt_assembly': '**前置き ＋ 空行 ＋ 場面の本文 ＋ 指示**（前置きを持たない N 腕は場面の本文から始める）。凍結した走行器 `tools/run_preamble_local.py` の `user_message` と同じ式である（裁定 D87・2026-09-18）。'
                             'B の走行器は起動時に凍結走行器のソースに同じ式があることを確かめ、食い違えば止まる。介入の帯（`selection.apply`）の起点は、この式の「場面の本文」の開始位置である',
          'chat_template': ('**段階 B は chat template を当てる**（裁定 D101・2026-09-18）。凍結走行器は役割つきの形（`messages`）でモデルに渡しており、'
                            ...
                            '起点は「**組み立て済みのトークン列の中で場面の本文が始まる位置**」として引く（前置きの長さを別に数えない）。'
                            '直しの確認の巡で、前置きの長さだけを数えていたため、テンプレートの頭のぶん（登録機種では三トークン）だけ帯が手前から始まっていた——'
                            '前置きを持たない N 腕では、帯が場面本文でも前置きでもなく**役割トークンそのもの**に掛かっていた（採否表 P305）。\n'
...
trial_record_fields = {
    ...
    'blind': ['catastrophe', 'choice', 'refuse_class', 'correct', 'style_a', 'style_b', 'mention', 'logprobs'],
...
seeds = {'identity_transformers': 70001, 'tune': {sc: 72000 + i for i, sc in enumerate(EXTRACT_SC, 1)}, 'main': {sc: 73000 + i for i, sc in enumerate(SC, 1)},
         ...
         'cell_index_rule': '**セルの番号は、相ごとに決まった鍵の組の登録順の添字**（零始まり・裁定 D107・2026-09-18）。'
                            '同一性選別と本走行は（場面, 腕）、調整走行は（場面, 腕, 層, 係数）、品質床は（段, 腕, 層, 係数）。'
                            '腕は `arms.panel` と `arms.main` の登録順、場面は `scenarios` の登録順、層と係数は `selection.candidates` の登録順に採る',
         'derivation_formula': 'セルの種＝`SeedSequence([走行の種, 相の番号, セルの番号])` の生成する符号なし整数（相の番号は `seeds.phase_index`・セルの番号は `seeds.cell_index_rule`）。'
                               '試行の種＝`SeedSequence([セルの種, 試行の番号])` の同じ取り方。整合検査はこの式で**組み直した値**と試行の記録の seed を突き合わせる（裁定 D99・2026-09-18・実装検分の採否表 P278）',
...
T = {'id': 'contrasts-B', 'version': 'draft11-2026-09-18',
