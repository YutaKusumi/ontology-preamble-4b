...
WHY = {
    ...
    'judge_validity.reading_check.code': ('照合記号の字と字数、記号の決め方と置き場、照合外れの割合の分母、やり直し待ちのあいだの採点の扱いが、登録者裁定 D42 の文言だけでは決まっていなかった。', '記号の無い行を照合外れに数えない案（記号を写さない書き出しを素通しにする）。分母をラベルのある件数にする案（ラベルの無い断片が多いほど割合が上がりにくい）。', NOLABEL),
