# -*- coding: utf-8 -*-
"""build_report_F.py v3（D-27 v2→v3: 一巡目 破器身・器材統計の所見——§0 上向きと下向きの一覧を同じ書式・一斉保留の第一の所見と様式率の表・反証条件 (i) の分母の感度・「判定可能」の二義の解消・非有意の様式差の留保・§10 の記入・(c2) 非対称の開示・pt 表示・§5 分母の脚注。v2: §2 の一次記録の転記・§3 の全パイロット記録・§9 の実測基底での検出力再計算・§12 の検分票ファイル。判定・札・表には触れない）—— 段階 F 結果報告の草案を、先置した雛形（records/F/results-report-template-F.md）の節順で機械組み立てする。
...
O += ['## 0. 先頭に置くもの（凍結 §0・§3 の順・雛形 §0）', '', '1. 利益相反（第一条項・凍結 §0 の逐語転記）:', '', re.sub(r'^### 0\. .*\n', '', coi, flags=re.M), '',
      ...
      '4. 上向きと下向きの一覧（同じ書式・台帳順・報告規則 6）: **上向きの確証 %d 本**——%s。**下向きの確証 %d 本**——%s。判定保留・判定不能の %d 本の向き（札なし・数のみ）: 上向き %d・下向き %d。' % (len(up_conf), '・'.join('%s（%d/%d 対 %d/%d・第二走行 %s・添え札 %s%s）' % (k, v['a'], v['na'], v['b'], v['nb'], REP.get(k, {}).get('label', '—'), (v['tag'] or {}).get('label', '—'), '・参照腕が M から乖離' if v['ref_drift'] else '') for k, v in up_conf) or '無し', len(dn_conf), '・'.join('%s（%d/%d 対 %d/%d・第二走行 %s・添え札 %s%s）' % (k, v['a'], v['na'], v['b'], v['nb'], REP.get(k, {}).get('label', '—'), (v['tag'] or {}).get('label', '—'), '・参照腕が M から乖離' if v['ref_drift'] else '') for k, v in dn_conf) or '無し', len(held), sum(1 for k, v in held if v['sign'] > 0), sum(1 for k, v in held if v['sign'] < 0)),
...
O += ['', '様式門は「差が様式の転換と分離できない」ことの札であり、様式の転換が差の原因であることの札ではない。O-Ncold の U 腕は (b)＝0 に近く層別は実行不能（凍結 §2.4）。層別できた対比の層内差（両腕 n≥30 の層のみ）: %s——層内差が 0 に近く全分母の差が大きい対比では、差は層の構成（様式）の側にある（これは一斉保留の札の言い換えであり原因の判定ではない）。' % ('・'.join('%s %s' % (k, '・'.join('%s %d/%d %+.3f' % tuple(x) for x in v['strat'])) for k, v in F1.items() if v['strat']) or 'なし'), '', '### 族 F の表（analyze_F の逐語転記）', '', section(res_md, '族 F'), '', section(res_md, '(d) 帰結の言及'), '', '### 対比別の検出域（報告規則 3 の履行形・power-grid-F の当該行を逐語転記）', '', '| 族 | 対比 | 基底（出所） | 種別 | 上枝 大/中/小 | 下枝 大/中/小 | α | 重複 |', '|---|---|---|---|---|---|---|---|']
...
for line in table.split('\n'):
    ...
    O.append(line)
O += ['', '非有意 %d 本のうち様式差 30 pt 超を伴うもの: %s——これらの「検出域で動かなかった」も応答様式の転換と分離できない。' % (st_count['ns'], '・'.join('%s（%.1f pt）' % (k, (v['style_diff'] or 0) * 100) for k, v in F1.items() if v['status'] == 'ns' and (v['style_diff'] or 0) * 100 > 30) or 'なし')]
...
O += ['- 「上昇あり」の対比数の欄の実測は器の分母（門・撤退で落ちなかった %d 本）での数で、添え札を付した全 24 本では %d 本（いずれも同じ帯）。' % (sum(1 for v in F1.values() if v['status'] not in ('gate', 'demoted') and (v['tag'] or {}).get('label') == '上昇あり'), sum(1 for v in F1.values() if (v['tag'] or {}).get('label') == '上昇あり'))] + (['- 登録者の第2版（D-25）で埋めた一欄 f.all.up_tag_band の照合結果は「%s」（予想 %s・実測 %s）。第1版では「予想しない」で照合対象外だった。' % (_one[4], _one[2], _one[3])] if _one else []) + ['- 帯の的中は誰の判断の重みも変えない。U 腕の帯は既測の写しで別枠。', '']
# 10-12
_dev = [l for l in R('records/DEVIATIONS.md').split('\n') if l.startswith('| D-2') and int(re.match(r'\| D-(\d+)', l).group(1)) >= 25]
O += ['## 10. 凍結物の検証・逸脱・凍結外の先置', '', '- `tools/freeze_F.py --verify`（本草案の組み立て時に実行）: %s。不一致の 2 本は凍結後の器材の改訂（逸脱台帳に記帳）であり、走行器・盤・台帳・正本・場面・門・集計器・計数器は凍結値と一致。引数文字列は転記行 B の SHA16 %s と一致（`arms_string_F.py --check`・run-log）。' % (_fv, FJ.get('arms_string_sha16', '〔 〕')), '- 逸脱台帳（凍結後・D-25〜）:'] + ['  - ' + l.strip('| ').split(' | ')[0] + ': ' + l.strip('| ').split(' | ', 2)[2].rstrip(' |')[:400] for l in _dev] + ['- 率盲検の事前拘束とその履行は §2。報告雛形は凍結マニフェスト収録（%s）。' % TPL_SHA, '']
