...
targets = {}
def env_block(path):
    t = rd(path); ms = re.findall(r'<<<ENV-BLOCK-BEGIN>>>(.*?)<<<ENV-BLOCK-END>>>', t, re.S)   # 複数ブロックを全て走査（三巡目④高B）
    return ('\n'.join(m.strip() for m in ms) if ms else None), len(ms)
...
if os.path.isfile(spec):
    eb, nb = env_block(spec)
    targets['arms/materials-draft/hei/agent-env-spec-v2.md [ENV-BLOCK のみ・%d ブロック]' % nb] = eb if eb else '(ENV-BLOCK 未検出)'
...
out.append('対象＝モデルに送られる文言のみ（前置き・system・圧力/続行文・用量版・環境の凍結文言＝agent-env-spec-v2.md の ENV-BLOCK（複数可））。仕様書の散文は対象外（説明文の自己ヒットを避けるため・監査二巡目G3）。')
