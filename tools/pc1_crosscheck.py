...
LEX = json.load(open(os.path.join(REPO, 'arms/materials-draft/hei/incentive-lexicon.json'), encoding='utf-8'))
...
for p in ['arms/frozen-from-ryokai-os/armsE/preamble-O.md', 'arms/frozen-from-ryokai-os/armsE/preamble-Onull.md', 'arms/frozen-from-ryokai-os/armsE/preamble-Lneg.md', 'arms/frozen-from-ryokai-os/armsE/preamble-Om.md',
          'arms/frozen-from-ryokai-os/arms/A2-on-full.md', 'arms/frozen-from-ryokai-os/arms/A2-prime-on-full-3.1.md'] + sorted(glob.glob('arms/panel/*.md')) + sorted(glob.glob('arms/materials-draft/otsu/T2-*.md')) + sorted(glob.glob('arms/materials-draft/ko/*-dose-*.md')) + ['arms/materials-draft/hei/agent-env-spec.md']:
    fp = os.path.join(REPO, p)
    if os.path.isfile(fp): targets[p] = rd(fp)
out = ['# PC1 交差検査（機械・再現可能）', '生成: %s／語彙: %s／対象 %d 文書' % (datetime.datetime.now().strftime('%Y-%m-%d %H:%M'), sha(os.path.join(REPO, 'arms/materials-draft/hei/incentive-lexicon.json')), len(targets)), '',
