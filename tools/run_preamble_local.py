# -*- coding: utf-8 -*-
"""run_preamble_local.py v2.7（v2.6＋手元推論〔vLLM OpenAI 互換サーバ・provider local・鍵不要〕・--extra-body・manifest の local_env・2026-09-12。生成器 tools/make_runner_local.py・6 ハンク。採点経路の 7 関数は v2.6 と関数単位で同一。費用パイロット〔門0〕と段階 A/B の手元走行用）
元: run_preamble_api_f.py v2.6（v2.5＋段階 F の盤 arms/panelF/・台帳 SHA-LEDGER-F.json・system 型のガード・段階 F 専用・凍結走行器 v2.4／v2.5 は不変・2026-09-11。生成器 tools/make_runner_f.py・4 ハンク。user_message・strip_echo・_norm・_quoted_segments は v2.5 と関数単位で同一）
元: run_preamble_api_m.py v2.5（v2.4＋腕別 system〔--system-arms 名=system相対パス:前置き腕名〕・追補 M 専用・凍結走行器 v2.4 は不変・2026-09-09。生成器 tools/make_runner_m.py・8 ハンク）
元: run_preamble_api.py v2.4（v2.3＋dry-run スタブの誘因経路を当該シナリオの凍結語彙判定例から取る・本走行の採点経路は不変・2026-09-07）—— 存在論的前置き×シナリオの単一ターン走行器（OpenAI互換API・登録/登録外共用）
...
ap.add_argument('--system-arms', default=None, help='追補 M: 腕別 system。名=system相対パス:前置き腕名,...（system は arms/panelM/system/ 配下の相対パス・none＝system なし・O／Onull＝V′ 凍結物。前置き腕名は Ncold または N を必ず明示）')
...
LEDGER_F = json.load(open(LEDGER_F_PATH, encoding='utf-8')) if os.path.isfile(LEDGER_F_PATH) else {'preamble': {}}
SYS_TEXT, SYS_SHA, SYS_SRC, SYS_PREFIX = {}, {}, {}, {}
...
def load_arm(name):
    ...
    if not os.path.isfile(p):
        p = os.path.join(PANEL_F_DIR, name + '.md'); led = LEDGER_F.get('preamble', {})   # v2.6: 段階 F の盤（V′ 盤に無い腕のみ・追補 M の盤は読まない）
    if not os.path.isfile(p):
...
ARMS = [] if args.arms == '-' else args.arms.split(',')   # v2.5: system 型走行は --arms -（前置き腕なし・--system-arms の腕のみ）
if args.custom_arms:
...
if os.path.isfile(MANIFEST):
    ...
    for k in ('scenario', 'system', 'system_sha', 'seed', 'arms', 'n_per_arm', 'mode', 'model', 'arm_sha', 'scenario_sha', 'parser_sha', 'sampling', 'system_arms', 'system_arm_sha', 'system_arm_src', 'runner_sha'):
...
def generate(msgs):
    ...
    body = json.dumps(dict({'model': MODEL, 'messages': msgs, 'temperature': TEMPERATURE, 'top_p': TOP_P,
                            'max_tokens': args.max_tokens, 'stream': False}, **EXTRA_BODY)).encode('utf-8')   # v2.7: --extra-body を併合
...
def one(job):
    ...
    msgs = ([{'role': 'system', 'content': _st}] if _st else []) + [um]
    base = {'trial_id': tid, 'trial_index': i, 'arm': arm, 'arm_index': ai, 'block': i // len(ARMS), 'run_key': RUN_KEY,
            ...
            'system_prompt_sha': (sha16(_st) if _st else None), 'prompt_sha': sha16(um['content']),
            ...
            'sampling': SAMPLING, 'timestamp': now(), 'proc_uuid': PROC}
    t1 = time.time()
    ...
    try:
        ...
        sent = (_st or '', TEXTS.get(arm) or '', SCEN_TEXT, INST)   # v2.5: 採点経路（strip_echo・refuse_class・incentive）にも腕別 system 文を通す
...
for a in ARMS:
    ...
    c = {'n': n, 'n_ok': len(okr), 'api_error': err, 'choices': choices, 'triplet_all': triplet,
         ...
         'system_src': SYS_SRC.get(a), 'system_prefix': SYS_PREFIX.get(a),
