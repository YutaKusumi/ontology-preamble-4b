# -*- coding: utf-8 -*-
"""boot_cost_pilot.py v2.2 —— 費用パイロット（門0・計画案 v2.2 §5 1″）の Colab 起動スクリプト（v1 2026-09-12 → v2 2026-09-13・大日如来「第三章 Colab 運用の手法」の採用）。
...
CFG = dict(repo='https://github.com/YutaKusumi/ontology-preamble-4b.git', commit=os.environ.get('OP4B_COMMIT', 'main'),
           ...
           vllm=os.environ.get('OP4B_VLLM', ''), drive=os.environ.get('OP4B_DRIVE', '1') != '0', units_before=os.environ.get('OP4B_UNITS_BEFORE'), dry=DRY, phase=PHASE, boot='v2.2')
...
if not DRY:
    if health():
        ...
        cmd = [sys.executable, '-m', 'vllm.entrypoints.openai.api_server', '--model', MODEL_DIR, '--served-model-name', CFG['model'], '--dtype', 'bfloat16',
               '--max-model-len', str(CFG['max_model_len']), '--gpu-memory-utilization', '0.90', '--port', '8000', '--seed', '0']   # v2.2: --disable-log-requests は vLLM 0.29 で廃止（要求ログは既定で無効）
