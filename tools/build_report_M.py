# -*- coding: utf-8 -*-
"""build_report_M.py v6 —— 追補 M 結果報告の草案を、先置した雛形（records/M/results-report-template-M.md・SHA16 56D74098321F299B）の節順で機械組み立てする。
...
if post:
    ...
    P('')
    P('| 族 | 対比 |' + post.split('| 族 | 対比 |', 1)[1].strip())
else:
...
P('- 凍結 §4「果たさないこと」の再掲（雛形 §13 の義務・独立項）: 4B 一機種・プロンプト層・単発。登録者の実感は測らない。F1「として句」は登録者の招請文の縮約ではない。M-b は F1 と Ncold の語形一致の交絡を統制しない。統制腕（PS・PK・MS・MK・F4）は音韻・文法・主題を揃えず、本結果では有意味列（MS・MK）自体が大きく動いた（§9）。抽出検査では、有意味列が応答の主題として取り込まれる型と、F4 の形式が名の枠を打ち消す向きに読まれる型を観察した（`sampling-inspection-M-2026-09-11.md`）——F4 は「名の枠が乗らない対照」ではなく「名の枠を否定する対照」として働きうる。測れないこと (a) 招請 対 ロールプレイは本設計の対比ではない／(b) L 腕が床近傍のとき末尾の効果は差の検定で測れない（sysLAmi）／(c) Ncold なしの腕の低さは基底の挙動。')
