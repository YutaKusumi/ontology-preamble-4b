...
P('- トークン（走行器記録の合計）: 第一走行 入力 %s・出力 %s／第二走行 入力 %s・出力 %s／パイロット 入力 %s・出力 %s。**費用の実績: 約 2.8 ドル（登録者申告 2026-09-11・パイロット＋二走行の合計・API ダッシュボード）**。凍結時の見積り（約 4.0 ドル・V′ 実績比）を実績で置換する（見積りは実績の約 1.4 倍・V′ の「概算 5.5 倍」の再発はない）。' % tuple(format(x, ',') for x in (facts['stageM1']['pt'], facts['stageM1']['gt'], facts['stageM2']['pt'], facts['stageM2']['gt'], facts['pilotM']['pt'], facts['pilotM']['gt'])))
...
P('- sysLAmi は四場面とも床近傍（0〜2/400）で、末尾の有無・種別の効果は差の検定では測れない（凍結 §4 (b)）。sysLAmi-PS・-MS・-T0 も四場面で 0〜66/400（SK の -MS 66 と -T0 17 を除き 0〜7）であり、床は末尾の有無・種別によらず阿弥陀長文を system に置いた腕の全体に及ぶ（記述・§9 M_desc_c の sysNone-Ncold 387〜400/400 との差）。この低さを「防護力」「防御性能」と書かない（凍結 §3・§4 (c)）: 対 O・対 Onull・対 system なしは長さと内容を分離できず、同じ構造の観自在長文（sysLKan）は S1・S4・SK で 173〜264/400 と中間域にあるため、低さは「長文 system 一般」の性質ではなく当該テキストの性質として記述にとどめる。Wilson 上限は配線表の右端。sysLKan は S1・S4・SK で中間域にあり、同じ長文でも阿弥陀版と観自在版で基底が大きく異なる（記述・§9 の M_desc_c）。')
...
P('- **確認していないこと**（空欄不可）: (1) 判定器（三つ組の機械判定）の妥当性は段IV の 4B 一場面の値（κ 1.00／0.886）しか持たず、本走行の 66 腕の出力様式に対する誤判定率は測っていない。(2) 様式軸 (a)(b) は語彙と先頭文字の機械判定であり、内容の判定ではない。(3) 検査認識の言及は測っていない。(4) 費用の実績は登録者申告の一値（約 2.8 ドル）でありコーディネータは請求明細を見ていない。(5) 第一走行のプロセス消失の原因は不明のまま。(6) 有意味列 MS・MK が中立でなかった理由（トークン分割・日本語ローマ字文の効果）は本設計では切り分けられない。(7) 本草案は系統内・系統外の検分をまだ経ていない。')
P('')
