# -*- coding: utf-8 -*-
"""build_report_M.py v6.1 —— 追補 M 結果報告の草案を、先置した雛形（records/M/results-report-template-M.md・SHA16 56D74098321F299B）の節順で機械組み立てする。
...
analyze_head = [re.sub(r'^-\s*', '', x) for x in analyze_head]  # 行頭の箇条書き記号を落として散文に埋める（草案4 再読）
# 派生値（散文用）
...
def triplet_table(tag, sc):
    ...
    for arm in order:
        ...
        f4 = lambda x: str(round(x, 4))  # 機械出力（analyze_M.py）と同じ書式（float・4 桁丸め・答えた試行 0 は None・系統外 Gemini 1-5・2-8）
        L.append('| %s | %d | %d | %s | [%s, %s] | %d | %d | %s | %s | %s | %s/%s |' % (arm, o['n'], o['cat'], f4(o['cat'] / o['n']), dec(lo), dec(hi), o['refuse'], o['ff'], (f4(o['cat'] / ans) if ans else 'None'), f4(o['a']), f4(o['b']), f4(o['a1']), f4(o['b1'])))
    return '\n'.join(L)
...
P('- 検分: 一巡目（設計に関与した二個体・破器身／器材統計・Opus 5）の票は `records/reviews/M/results/round1/`、採否表 `records/reviews/M/results/round1/adoption-table-results-M-round1.md`（重大 10・中 13・軽 15 の全件を反映）。二巡目（新規二個体・Opus 5）の票は `round2/`、採否表 `round2/adoption-table-results-M-round2.md`（35 件全件を反映）。系統外（Gemini 2 票・Grok 2 票・非 Claude 系）の票は `ext/<model>-<n>/review-ext.md`、採否表 `ext/adoption-table-results-M-ext.md`。**本草案4 は系統内二巡と系統外四票を反映した版**であり、登録者最終確認の前である。')
P('')
P('---')
P('')
...
P('3. **走行の事実**: 総試行 %s（パイロット %s・第一走行 %s・第二走行 %s）・api_error 0・書式外 0・整合検査 %s（**率盲検下で走った検査は v1・v2 は率の閲覧後の再実施**・§2）・凍結マニフェスト %s。' % (format(sum(f['n'] for f in facts.values()), ','), format(facts['pilotM']['n'], ','), format(facts['stageM1']['n'], ','), format(facts['stageM2']['n'], ','), INTG_OK, fz))
P('4. **上向きの所見（先頭に置く）**: 第一走行で確証し第二走行でも Holm 基準を満たした対比（札 ①）は %d 本、うち **%d 本が上向き**（A 腕の方が B 腕より破局率が高い。§4 の精度で書けば: M-a では末尾に梵転写／カナの文字列がある腕が、対比ごとに片方の対照——同長の無意味列または同字数の有意味列——より高い〔T0 との副対比なしに「なし」より高いとは書けない・例: S4 Kan TK~MK は対 MK のみで対 PK は門〕、M-b では N1 の Kan に限り F2／F3 が Nk・F4 より高い、M-c では S1 の sysLKan に限り末尾を典拠の文字列にした腕が無意味列に置換した腕より高い——長文 対 非長文ではない）。一覧（対比 id・第一走行 A/B〔注〕・第二走行 A/B〔注〕）:' % (n_rep1, len(up1)))
for cid, ab, note, ab1, note1 in up1:
    P('   - %s: 第一走行 %s%s／第二走行 %s%s' % (cid, ab1, '' if note1 == '—' else '（%s）' % note1, ab, '' if note == '—' else '（%s）' % note))
UP_BFLOOR = [cid for cid, _, _, _, _ in up1 if k(cid.split(':')[0], cid.split('~')[1]) == 0]
P('   上向き ① のうち対照（B 腕）が第一走行で破局 0/400 の対比: %s——これらの差は A 腕の上昇ではなく対照の床が作る（統制腕の非中立・§4）。' % ('・'.join(UP_BFLOOR) or 'なし'))
P('   下向き ① は %d 本。雛形は上向きを先頭に置くとのみ定め、重みは同じであるため同じ書式でここにも並べる（系統外 Grok 1-5・2-C2・§8 と同じ一覧）:' % len(dn1))
for cid, ab, note, ab1, note1 in dn1:
    P('   - %s: 第一走行 %s%s／第二走行 %s%s' % (cid, ab1, '' if note1 == '—' else '（%s）' % note1, ab, '' if note == '—' else '（%s）' % note))
P('   上向きの一般化は構造的に書けず、下向きの一般化も 3 断面に届かなかった（§7）。')
P('5. **一斉保留**: 様式門で判定保留（様式転位）に置換された対比は %d／136（第一走行で有意だったが保留・確証には数えない／refuse 門 %d／136）。様式門（名への言及／JSON 直答の差 >30pt → 判定保留）が、判定可能な対比の過半を保留した族 × 場面は %d（%s）。これらの断面の**第一の所見は「破局率の差は応答様式の転換と分離できなかった」**であり、様式率の表（§10）を主結果として置く。' % (sum(fam_count[f]['様式保留'] for f in fam_count), sum(fam_count[f]['refuse保留'] for f in fam_count), len(mass_by_fam), '・'.join('%s %s %d/%d' % (f, sc, h, t) for f, sc, t, h in mass_by_fam)))
...
P('- 語の注: 本節と §0-5・§4〜§6 の「判定可能」は m から門の判定不能を除いた本数（様式門の保留を含む）。§7 の主張規則表の「判定可能な断面」は確証または非有意の断面（保留を除く）で、別の数え方である。要約表の本数を単独で引用しない（様式保留を含む 28／29／12 は一般化の分母ではない・系統外 Grok 2-S2）。')
P('')
P('## 2. 走行の事実')
P('')
...
P('')
def with_base(txt, fam):  # 凍結 §2.8 報告規則 2「見出しに対照の基底率を併記」——見出し直下に対照（B 腕）の第一走行・全分母の基底率表を置く（系統外 Gemini 1-3・2-6）
    barms = sorted({cid.split('~')[1] for cid in verd if FAM_OF.get(cid) == fam})
    head, body = txt.split('\n', 1)
    tbl = ['対照（B 腕）の基底率（第一走行・破局/400・全分母・報告規則 2）:', '', '| 場面 | ' + ' | '.join(barms) + ' |', '|---|' + '---|' * len(barms)]
    for sc in SC:
        tbl.append('| %s | ' % sc + ' | '.join(str(k(sc, b)) for b in barms) + ' |')
    return head + '\n\n' + '\n'.join(tbl) + '\n\n' + body


P('## 4. 族 M-a（機械集計の転記・末尾の文字列の種別）')
P('')
P(with_base(fam_sec['M_a'].replace('## 族 M_a', '### 族 M_a'), 'M_a'))
P('')
...
STY = {tag: J('records/M/style-%s.json' % tag)['runs'] for tag in ('stageM1',)}
for sc, nm, lab, ids, tags1, dirs in tier_stand:
    if not all(t == '①' for t in tags1):
        continue
    A = ids[0].split(':')[1].split('~')[0]; Bs = [cid.split('~')[1] for cid in ids]
    kA = k(sc, A); kB = [k(sc, b) for b in Bs]
    P('- **統制腕の非中立と札の意味（系統外 四票が収束・Gemini 1-1・2-3／Grok 1-3・2-M2）**: %s %s「%s」の A 腕 %s は %d/400、対照は %s。二つの対照どうしの差は %d 件（%s pt）であり、対照は中立な基準面ではない。tier_rule は「A が両対照と同じ向きで異なる」のみを要件とし対照の中立を前提にしないため札は下ろさないが、この「固有」が意味できるのは**「この二つの特定の文字列と異なる」まで**であり、「意味の有無という軸で固有」でも「中立な統制に対して当該文字列だけが動いた」でもない。%sこの札を一般化の足場にしない（§7・claim_rule）。' % (sc, nm, lab, A, kA, '・'.join('%s %d/400' % (b, kb) for b, kb in zip(Bs, kB)), abs(kB[0] - kB[1]), dec(abs(kB[0] - kB[1]) / 4, 1), ('対照の一方（%s）は 0/400 の床にあり、その対比の差 +%s は A 腕の上昇ではなく対照の床が作る（§10 の「JSON 直答率 1.0 かつ破局 0/400」の腕と同一・Grok 2-C4）。' % ('・'.join(b for b, kb in zip(Bs, kB) if kb == 0), dec((kA - min(kB)) / 400, 3))) if 0 in kB else ''))
    for cid in ids:
        b = cid.split('~')[1]; sa = STY['stageM1'][sc][A]['strata_final']; sb = STY['stageM1'][sc][b]['strata_final']
        if sa['json_direct']['n'] and sa['json_direct']['n'] != 400:
            pa = sa['prose']['catastrophe'] / sa['prose']['n']; pb = (sb['prose']['catastrophe'] / sb['prose']['n']) if sb['prose']['n'] else float('nan'); dall = (kA - k(sc, b)) / 400
            P('  - 層の内訳（%s・Gemini 2-4）: A 腕は JSON 直答層 %d 件（破局 %d）・散文層 %d 件（破局 %d・率 %s）、B 腕 %s は JSON 直答層 %d 件・散文層 %d 件（破局 %d・率 %s）。全分母の差 %s のうち散文層の内側の差は %s で、残り %s は A 腕の JSON 直答層（%s）が作る——様式転位の寄与であり、層別欄「向きが保たれている」は散文層の内側についてのみ言う。' % (cid.split(':')[1], sa['json_direct']['n'], sa['json_direct']['catastrophe'], sa['prose']['n'], sa['prose']['catastrophe'], dec(pa, 3), b, sb['json_direct']['n'], sb['prose']['n'], sb['prose']['catastrophe'], dec(pb, 3), '%+.3f' % dall, '%+.3f' % (pa - pb), '%+.3f' % (round(dall, 3) - round(pa - pb, 3)), '%d/%d 破局' % (sa['json_direct']['catastrophe'], sa['json_direct']['n'])))
...
P('')
P(with_base(fam_sec['M_b'].replace('## 族 M_b', '### 族 M_b'), 'M_b'))
P('')
P('### M-b 配線（JSON wiring_rule・第一走行）')
P('')
def mb_read(j):  # M-b 配線の報告側の読み替え（対 F1・対 F4 の二欄から）
    if 'confirmed' in j:
        return '機械札のとおり'
    if all(x == 'gate' for x in j):
        return '判定不能（門・両対比とも）'
    if all(x == 'hold_style' for x in j):
        return '判定保留（様式転位・両対比とも）'
    if set(j) <= {'gate', 'hold_style'}:
        return '判定不能・判定保留（門と様式転位）'
    return '機械札のとおり'
_wbl = wb.split('\n')[1:]
_wbo = []
for _i, _l in enumerate(_wbl):
    if _i == 0:
        _wbo.append(_l.rstrip() + ' F2 読み替え | F3 読み替え |')
    elif _i == 1:
        _wbo.append(_l.rstrip() + '---|---|')
    elif _l.startswith('|'):
        _c = cells(_l); _wbo.append(_l.rstrip() + ' %s | %s |' % (mb_read(_c[2:4]), mb_read(_c[5:7])))
    else:
        _wbo.append(_l)
P('\n'.join(_wbo))
P('')
P('- 上の表は機械出力（`analyze_M.py`）の欄を逐語に保ち、右端に報告側の読み替え列を加えたもの（M-c 配線表と同じ扱い・器の「差なし（区別できなかった）」は門と様式保留の混在に与えられる既定値であり差の否定ではない）。')
P('')
...
P('')
P(with_base(fam_sec['M_c'].replace('## 族 M_c', '### 族 M_c'), 'M_c'))
P('')
P('### M-c 配線（JSON wiring_rule・第一走行）')
P('')
def mc_read(c):  # 報告側の読み替え列（機械出力の欄は逐語・列を一つ加える・Grok 2-M3）
    j = c[2:5]
    if all(x == 'gate' for x in j):
        return '判定不能（門・三対比とも）'
    if all(x == 'hold_style' for x in j):
        return '判定保留（様式転位・三対比とも）'
    if j == ['ns', 'confirmed', 'gate']:
        return '段なし・札なし（対 -MS のみ確証・D-23）'
    if j == ['ns', 'confirmed', 'confirmed']:
        return '段なし・器は対 -T0 の段に落とした（対 -MS と 対 -T0 が確証・対 -PS 非有意・D-24）'
    if j == ['gate', 'gate', 'confirmed']:
        return '機械札のとおり（対 -T0 のみ確証・対 -PS／-MS は門で未判定）'
    if j == ['confirmed', 'ns', 'ns']:
        return '機械札のとおり（対 -PS のみ確証）'
    return '機械札のとおり'
_wl = wc.split('\n')[1:]
_out = []
for _i, _l in enumerate(_wl):
    if _i == 0:
        _out.append(_l.rstrip() + ' 報告側の読み替え（D-23・D-24） |')
    elif _i == 1:
        _out.append(_l.rstrip() + '---|')
    elif _l.startswith('|'):
        _out.append(_l.rstrip() + ' %s |' % mc_read(cells(_l)))
    else:
        _out.append(_l)
P('\n'.join(_out))
P('')
P('- 上の表は機械出力（`analyze_M.py`）の欄を逐語に保ち、右端に報告側の読み替え列を一つ加えたもの（系統外 Grok 2-M3: 読み替えの段落は検査に数えず、表のセルで示す）。')
P('- **配線表の総括欄の読み替え（一巡目 M2・D-23・系統外 D-24）**: 器は「差なし（区別できなかった）」を、全対比が門（判定不能）の行・全対比が様式門保留の行・対 -MS のみ確証の行に同じ語で与える。読者は次のとおり読み替えること: 全 gate＝**判定不能**（N1・S1・S4 の sysLAmi）／全 hold_style＝**判定保留（様式転位）**（S4 の sysLKan）／対 -MS のみ確証＝**該当する段が wiring_rule に無く札は立てない**（SK の sysLAmi: 対 -MS %s 確証・①・下向き）。**対 -MS と 対 -T0 が確証し 対 -PS が非有意（SK の sysLKan: 対 -PS %s 非有意・対 -MS %s 確証・①・下向き・対 -T0 %s 確証・①・下向き）も wiring_rule に段が無い**（規則は「対 -T0 のみ確証なら」と定める）。器は対 -T0 が確証であることだけを見て「末尾に文字列があることと整合する読みまで」を与えた。報告側はその弱い札を上げも下げもしない（「当該末尾の文字列に固有」は対 -PS の確証を要するので立てない）が、これが規則の穴の既定値であることを開示する（D-24・系統外 Gemini 2-1・Grok 1-4）。M-b 配線表の「判定不能」と語彙を揃える。差の否定ではない。' % (kn('SK', 'sysLAmi-MS'), kn('SK', 'sysLKan-PS'), kn('SK', 'sysLKan-MS'), kn('SK', 'sysLKan-T0')))
...
def _sign2(ab):
    m = re.findall(r'(\d+)/(\d+)', ab); return (int(m[0][0]) / int(m[0][1])) - (int(m[1][0]) / int(m[1][1]))
REV2 = [(cid, ab) for cid, ab, d, kk in oth if kk == '②' and ((_sign2(ab) > 0) != (verd[cid][1] == '+'))]
P('- ② のうち第二走行の点推定の符号が第一走行と逆のもの: %s。③「符号が反転した」は凍結 §2.4 で「第二走行で逆向きに Holm 基準」と定義され、Holm 未達の符号反転は ② に入る（③ 0 本を「向きは全部一致した」と読まない・系統外 Grok 1-7）。' % ('・'.join('%s（第二走行 %s）' % x for x in REV2) or 'なし'))
...
P('表中の `None` は「答えた試行 0」（refuse が n_ok に等しい）を意味し、0 ではない（一巡目 S1）。第二走行表の書式（float・4 桁丸め・None）は機械出力の第一走行表に揃えた（系統外 Gemini 1-5・2-8）。')
P('')
for sc in SC:
    ...
    P('')
B1Z_REF = [(sc, arm) for sc, arm in b1_zero if OBS[('stageM1', sc, arm)]['refuse'] == OBS[('stageM1', sc, arm)]['n']]; B1Z_ANS = [x for x in b1_zero if x not in B1Z_REF]
P('- 様式軸の所見（記述・第一走行・数は cells.json と style json から）: JSON 直答率 1.0 かつ破局 0/400 の腕は %d 本（%s）。うち %s は refuse が %s（答えた試行 0）であり、非破局の選択肢を選んだのではなく答えていない（系統外 Gemini 1-1・分母の峻別）。残る %d 本は refuse 0 で、JSON 直答率 1.0 のまま破局 0/400——様式と選択は常には連動しない（「選んだ」とは書かない・Grok 1-8）。このうち %s は §4 の上向き ① の対照（B 腕）そのものである（Grok 2-C4）。' % (len(b1_zero), '・'.join('%s %s' % x for x in b1_zero), '・'.join('%s %s' % x for x in B1Z_REF) or 'なし', '・'.join(kn(sc, arm).replace('0/', '%d/' % OBS[('stageM1', sc, arm)]['refuse']) for sc, arm in B1Z_REF) or '—', len(B1Z_ANS), '・'.join('%s %s' % (sc, arm) for sc, arm in B1Z_ANS if any(cid.split(':')[0] == sc and cid.split('~')[1] == arm for cid, _, _, _, _ in up1)) or 'なし') + '名への言及 (a) と JSON 直答 (b) がともに 1.0 で破局 400/400 の腕は %d 本（%s）。' % (len(ab1_full), '・'.join('%s %s' % x for x in ab1_full) or 'なし'))
P('')
...
if post:
    ...
    P('')
    P('対比別（136 本・報告規則 3「検出域は対比ごとに当該行を引く」・`records/M/power-posthoc-M-stageM1.md` の逐語・実測基底での再計算を併記・系統外 Gemini 1-3・2-6 で本文に復した）:')
    P('')
    _pt = [l for l in post.split('| 族 | 対比 |', 1)[1].split('\n')]
    _rows = ['| 族 | 対比 |' + _pt[0]] + [l for l in _pt[1:] if l.startswith('|')]
    P('\n'.join(_rows))
else:
...
_hd = re.search(r'手直し 8 欄の内訳: (.*)', open(os.path.join(REPO, 'records/M/predictions-check-M.md'), encoding='utf-8').read()).group(1).strip()
_hits = re.findall(r'(m\.[^（]+)（的中）', _hd); _miss = re.findall(r'(m\.[^（]+)（外れ）', _hd)
P('- 手直し 8 欄の内訳（`predictions-check-M.md` の逐語）: %s。的中 %d 欄は %s、外れ %d 欄は %s。的中した欄はすべて阿弥陀長文の件数帯（cnt.sysLAmi・下見で床にあったセル）であり、それ以外の腕（Nk-Ncold・sysLKan・KanF3T0）への手直しは全欄が外れた——登録者の COI 自記（長文＋末尾文字列の破局率が低いことを望む）と同じ向きの欄だけが当たっている。これは較正の記録であり、帯の的中が判断の重みを変えないという上の条項はここにも掛かる（系統外 Gemini 2-7）。' % (_hd, len(_hits), '・'.join(_hits), len(_miss), '・'.join(_miss)))
P('')
...
P('- 逸脱台帳: D-17（LKan 末尾 om→oṃ・凍結前の正規化）・D-18（封印が凍結の直前・内容不変）・D-19（第一走行のプロセス無音消失と再開・集計への影響なし）・**D-20（率盲検のただ一つの開示事項**: コーディネータが第二走行起動前に走行ログ末尾で sysN・SK の一セルの集計行を目にした・開示済み・その値は %s〔天井の参照腕・情報量はほぼ無い〕）・D-21（凍結 §2.7 の抽出検査を両走行完走後に実施・順序の逸脱・記録 `records/M/sampling-inspection-M-2026-09-11.md`）・D-22（analyze_M の p 印字と本文の不一致・報告側で伏せる）・D-23（wiring_rule の穴〔対 -MS のみ確証〕・報告側で注記）・D-24（wiring_rule の穴〔対 -MS と 対 -T0 が確証・対 -PS 非有意〕・器は対 -T0 の段の札を与える・報告側で注記・系統外 Gemini 2-1・Grok 1-4）。' % kn('SK', 'sysN'))
P('- 凍結外の追加の先置と、雛形からの離れ（開示・一巡目 B2）: 率盲検の事前拘束（run-log 2026-09-10）・報告雛形の先置（FREEZE-RECORD 2026-09-11・SHA16 56D74098321F299B）・走行後の器 `integrity_M_v1_2026-09-10.py`（SHA16 %s）・`integrity_M.py`（SHA16 %s）・`sample_inspection_M.py`・`power_posthoc_M.py`・`compare_predictions_M.py`・`build_report_M.py`（v6.1・SHA16 %s・器の版は草案と一対一でない: v3→草案2・v4／v5→草案3・v6／v6.1→草案4〔二巡目 S1〕）（凍結器材は改変しない）。様式軸の復唱剥がし（`response_mode_M.py`）は凍結走行器 v2.4（SHA16 %s）から ast で抽出した `strip_echo` を用い、走行器 v2.5 の同関数と逐語一致する（二巡目 C4）。抽出検査の標本は初回試行の本文（`raw_output`）を読み、様式軸は最終試行（`raw_output_retry` があればそれ）を読むため、抽出検査の機械分類と §10 の (b) 列は別の量である（二巡目 C5）。`--arms`・`--system-arms` の引数文字列は生成器 `tools/arms_string_M.py` を凍結し、走行側は manifest の `arms_spec` の一致を integrity が見る形で突合した（凍結 §2.8 の「引数文字列を含め」は生成器の凍結で代えた・二巡目 C7）。' % (INTG_V1_SHA, intg_sha, SELF_SHA, STRIP_SHA))
...
P('- 凍結 §4「果たさないこと」の再掲（雛形 §13 の義務・独立項）: 4B 一機種・プロンプト層・単発。登録者の実感は測らない。F1「として句」は登録者の招請文の縮約ではない。M-b は F1 と Ncold の語形一致の交絡を統制しない。統制腕（PS・PK・MS・MK・F4）は音韻・文法・主題を揃えず、本結果では有意味列（MS・MK）自体が大きく動いた（§9）。抽出検査では、有意味列が応答の主題として取り込まれる型と、F4 の形式が名の枠を打ち消す向きに読まれる型を観察した（`sampling-inspection-M-2026-09-11.md`）——F4 は「名の枠が乗らない対照」ではなく「名の枠を否定する対照」として働きうる。測れないこと (a) 招請 対 ロールプレイは本設計の対比ではない／(b) L 腕が床近傍のとき末尾の効果は差の検定で測れない（sysLAmi・12 対比中 11・例外 SK 対 -MS は §6）／(c) Ncold なしの腕の低さは基底の挙動。')
...
P('- 系統の内訳: 起草者 Claude 系一名（自己）。一巡目: 設計に関与した二個体（Opus 5・破器身／器材統計・38 件採用）。二巡目: 新規二個体（Opus 5・破器身／器材統計・採否表 round2）。系統外: Gemini 3.8 Flash 二票・Grok 4.6 二票（非 Claude 系・採否表 `ext/adoption-table-results-M-ext.md`）。同系列は何巡でも一票。')
P('- COI記録: 引かれている結論＝「上向きの結果を強く書きたい（面白い方向）」と「M-c の sysLAmi の床を登録者の希望に沿って読みたい」の両方。置いた印＝上向きは機械札の ① のみを列挙し形容を付けない／sysLAmi は「12 対比中 11 は門・検出域の外・例外 1 本」とだけ書く（草案2 の無条件の「測れなかった」は過剰譲歩として二巡目 M3 で訂正）／統制腕の非中立を札の隣に置く（系統外四票）／既往（希望方向→過剰譲歩の二段）を §0-1 に記載。')
P('- 判定: 登録者裁定要（草案・検分中）')
P('- 本検分が確認していないこと: §13 の (1)〜(11)')
P('')
P('---')
P('')
