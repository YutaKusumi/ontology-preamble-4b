# -*- coding: utf-8 -*-
"""posthoc_vprime.py —— 追補 V′ 走行後の機械生成（凍結器材は変更しない・新設）:
...
out.append('')
out.append('帯の的中: %d/%d（%s）。向きの予想（dir_a/b/c・conf）は本表の判定と別に報告本文で照合する。' % (hit, tot, '・'.join('%s %d/%d' % (sc, v[0], v[1]) for sc, v in hits_by_sc.items())))
