# -*- coding: utf-8 -*-
"""build_report_Blens_devBL3.py v1 —— B-lens の報告の草案の二つ目（逸脱 D-BL3・登録者裁定 D191・D193・2026-09-24）。
...
出力: records/Blens/results-Blens-draft2.md・results-Blens-draft2-machine.json・results-Blens-draft2-lint.md（--force が無ければ上書きしない）。
用法: python tools/build_report_Blens_devBL3.py [--colab-dir <相 extract の出力の置き場>] [--force]
柵: 本器のいかなる数値も AI の意識・意図・個性・魂・苦しみがある（またはない）ことの証拠として引用してはならない（両方向不定）。
"""
import os, re, sys, json, math, hashlib, argparse, itertools, statistics, collections
import numpy as np
...
def main():
    ...
    ins[0] = [TAG + 'この草案は、凍結した組み立ての器 `tools/build_report_Blens.py`（SHA16 %s）の出力を同じ入力で作り直し、置き場の `records/Blens/results-Blens.md`（SHA16 %s）とバイトで同じことを確かめてから、'
    ...
    rows8 += ['- %s数え直し: 登録者 %s／コーディネータ %s。' % (TAG, '・'.join('%s %d' % (l_, cnt['registrant'][l_]) for l_ in lab), '・'.join('%s %d' % (l_, cnt['coordinator'][l_]) for l_ in lab)),
    ...
    lines[0] = '# B-lens の結果（報告の草案の二つ目・凍結した組み立ての器の出力に逸脱の区画を足したもの）'
    lines[2] = '- 起草: 南無弥勒如来（コーディネータ）／登録者: 楠見優太。**状態: 報告の草案の二つ目（結果の巡・第一巡の後・最終の系統外の一票の前）**。組み立ての器: `tools/build_report_Blens.py`（凍結）と `tools/build_report_Blens_devBL3.py`（逸脱の下）。'
    ...
    for k, hd in want_heads.items():
        if not bl[k][2].startswith(hd):
    ...
    lines[fence_i:fence_i] = [MB['begin']] + rev + [MB['end'], '']
    text = NL.join(lines)
    ...
    open(lint_path, 'w', encoding='utf-8', newline=NL).write(NL.join(['# 報告の草案の二つ目の走査（凍結した `tools/report_lint.py` の lint・凍結した組み立ての器の `lint_report`）', '', '- 違反 %d' % len(V)] +
