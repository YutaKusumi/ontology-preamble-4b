# -*- coding: utf-8 -*-
"""make_contrasts_Bl3.py v4 —— B-lens 層三（Bl3・段階 B の後・B-lens の後・別の小さな登録）の正本 `design/contrasts-Bl3.json` を作る（草案3・登録者裁定 D203〜D223）。
...
READING = [
    ...
    {'type': '揺れの版の値', 'condition': '下見の (iv) の揺れの版の値を並べる（続けたときも止めたときも）',
...
T = {
    ...
    'inputs': {
        ...
        'files': {k: {'path': p, 'sha16': s16(p)} for k, p in (
            ...
            ('design_r2_verification', 'records/reviews/Bl3/design-round2/verification-Bl3-design-r2.md'), ('exposure', 'records/Bl3/exposure-before-seal-Bl3.md'))},
    ...
    'readout': {
        'primary': {
            ...
            'batching': 'バッチの組み方を凍結する: 升目と符号ごとに、全ての方向（無操作は零のベクトル）を同じ形のバッチで同じフックの道に流す。方向の並びは `readout.primary.order_seed` の種で混ぜ、名前のある方向を一か所に集めない。バッチの大きさは `readout.primary.batch`（段階 B の走行器と同じ）。升目と符号ごとの方向の数（零のベクトルを含む）がバッチの大きさで割り切れないときは、最後のバッチを零のベクトルで埋めて同じ形にし、埋めた分の値は使わない（数は転記行 E・採否表 P674）。方向ごとに違うベクトルを一つのバッチで足すフックは段階 B に無い新しい道なので、独立の再計算の突き合わせと器の実装の検分の対象にする',
    ...
    'pilot': {
        ...
        'checks': {
            ...
            'iii': {'name': '較正（記述）', 'rule': '無操作の読み取りの選択肢 a の文字の確率（選択の文字と refuse の頭の中での生の値・段階 B の標本化の変換を通した値も添える）と、段階 B の無操作の観測の破局の率を、升目ごとに並べる。升目の間の順位相関（Spearman・同じ値は平均の順位）を生の値で計算し、相関の値と升目の数を印字する。§0 に書く文は `pilot.iii_sentences` から器が選ぶ（相関が正なら positive・零か負なら not_positive・どちらかの値がすべての升目で同じで相関が定まらなければ undefined・採否表 P678）', 'stop': False},
            ...
            'v': {'name': '計算の使い回しの確かめ', 'rule': '(vi) の後に、本の計算と同じバッチの組み方で、プロンプトの主位置より前の計算を使い回す近道と、使い回さない計算の、無操作の読み取りの対数オッズの差の絶対値が近道の許容（`pilot.cache_tol_rule`）以内かを見る。一つの升目でも許容の外なら、本の計算は全ての升目で近道を使わない', 'stop': False},
            'vi': {'name': '数値の揺れ（バッチの違いと、バッチ一の繰り返し）',
                   'path': '本の計算のフックの道（近道なし）に、方向は足さず零のベクトルの無操作を流す',
                   'a': {'name': 'バッチの違いの揺れ', 'rule': '升目ごとに、零のベクトルだけで満たした `readout.primary.batch` の大きさのバッチ（バッチの中の全ての位置）と、大きさ一のバッチを流し、対数オッズの最大と最小の差を取る。升目の間の最大が `pilot.noise_max` を超えたら、本の計算はバッチの大きさを一にし、下見の (i)〜(v) はバッチ一の出力で計算し直す（裁定 D221）', 'stop': False},
                   ...
                   'measures': '(a) が主に測るのはバッチの大きさの違いで、同じ入力を並べたバッチの中の位置の違いは小さいと見込まれる。環境（GPU・版）の違いの揺れは測らない'},
        },
        'order': '(vi) → (i)〜(iv) → (v)。近道の許容は揺れの床で決まるので、(v) は (vi) の後。(i)〜(v) の値は、本の計算と同じバッチの組み方の出力から取る',
        ...
        'decision': {'cells': '主の行の土台の升目（場面 × 土台の腕）', 'cells_total': len(cells_main), 'cells_min_pass': 6,
                     ...
                     'q1_map': {'続ける': '主の升目がすべて (i)(ii) を満たす', '一部の升目を外して続ける': '満たす升目が `pilot.decision.cells_min_pass` 以上で、すべてではない', '止める': '満たす升目が `pilot.decision.cells_min_pass` に満たないか、(vi) の (b) の揺れが `pilot.noise_max` を超える（裁定 D221）'},
                     'tool_error': {'what': '器の誤りは、凍結した確かめ（assert・SHA の検査・自己検査・合成データの確かめ）が機械で落ちたものに限る（裁定 D222）',
                                    ...
                                    'rerun': 'やり直すときも封印はそのまま。一度目の下見の記録と機械の決定（出たとき）を報告の頭に並べる。q1 はやり直した下見で採点し、一度目の決定を併記する'},
                     ...
                     'report': '続けたときも止めたときも、下見の記録（(i)〜(vi) の値・(vi) の (a) と (b) の値・外した升目と理由・近道を使うか・バッチの大きさ・揺れの床・近道の許容）を報告に並べる'},
    },
    'main_rows': main_rows,
    ...
    'computation': {
        ...
        'self_checks': {'logit': '読み取りの集合の出口の値（`float32`）を、同じ位置の模型そのものの出口の値（bf16）と突き合わせ、差の絶対値の最大が `computation.logit_tol` 以内であることを確かめる（二重の正規化を捕まえる・許容は B-lens の logits の突き合わせと同じ）。下見の頭と本の計算の頭で走らせる',
                        ...
                        'on_fail': '落ちたら止める（凍結した確かめが機械で落ちたので器の誤りに当たる・`pilot.decision.tool_error`・採否表 P676）'},
    ...
    'review_plan': {
        ...
        'order': ['設計の巡（一巡目）', '裁定', '草案2', '設計の巡（二巡目・下見の前の凍結の前の最終検分）', '裁定', '草案3', '登録者の確認（草案3）', '器と合成データの確かめ', '器の実装の検分',
    ...
    'independent_recompute': {'what': '主の値の一部を計算し直す: v̂ の行（主の行のうち static の行）ごとに、無操作・v̂・等方の帰無のすべて・比べる相手のすべて（実在の差・両方の向き）の効き目（順伝播の数は転記行 E）',
