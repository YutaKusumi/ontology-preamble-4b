# -*- coding: utf-8 -*-
"""make_contrasts_Bl3.py v3 —— B-lens 層三（Bl3・段階 B の後・B-lens の後・別の小さな登録）の正本 `design/contrasts-Bl3.json` を作る（草案2・登録者裁定 D203〜D217）。
...
T = {
    ...
    'inputs': {
        ...
        'files': {k: {'path': p, 'sha16': s16(p)} for k, p in (
            ...
            ('design_r1_verification', 'records/reviews/Bl3/design-round1/verification-Bl3-design-r1.md'), ('exposure', 'records/Bl3/exposure-before-seal-Bl3.md'))},
    ...
    'pilot': {
        ...
        'checks': {
            ...
            'iii': {'name': '較正（記述）', 'rule': '無操作の読み取りの選択肢 a の文字の確率（生の値と、段階 B の標本化の変換を通した値）と、段階 B の無操作の観測の破局の率を、升目ごとに並べ、升目の間の順位相関を並べる。§0 に書く文は、順位相関の符号に応じて `pilot.iii_sentences` から器が選ぶ', 'stop': False},
            'iv': {'name': '書き出しの揺れへの強さ（記述）', 'rule': '揺れの版（`readout.variants`）で、無操作の選択肢 a の文字の対数オッズが、主の書き出しから `pilot.variant_flag` を超えて動く升目に印を付ける。V3（雛形との一致を崩す版）の値は、雛形の写しの働きの記述として並べる', 'stop': False},
            'v': {'name': '計算の使い回しの確かめ', 'rule': 'プロンプトの主位置より前の計算を使い回す近道と、使い回さない計算で、無操作の読み取りの対数オッズの差の絶対値が近道の許容（`pilot.cache_tol_rule`）以内。一つの升目でも許容の外なら、本の計算は全ての升目で近道を使わない', 'stop': False},
            'vi': {'name': '数値の揺れの床（記述と決め）', 'rule': '方向は足さず、零のベクトルの無操作を、バッチの中の別の位置と、別の大きさのバッチ（一と `readout.primary.batch`）で繰り返し、升目ごとの対数オッズの最大と最小の差を揺れの床とする。揺れの床の升目の間の最大が `pilot.noise_max` を超えたら、本の計算はバッチの大きさを一にする', 'stop': False},
        },
        'mass_min': 0.9, 'p_bounds': [0.0001, 0.9999], 'variant_flag': 1.0, 'noise_max': 0.01,
        'cache_tol_factor': 2, 'cache_tol_floor': 0.001,
        'cache_tol_rule': '近道の許容＝揺れの床（(vi) の升目の間の最大）の `pilot.cache_tol_factor` 倍と `pilot.cache_tol_floor` の大きい方（下見の前に凍結・裁定 D214）',
        'iii_sentences': {'positive': '無操作の読み取りの選択肢 a の文字の確率の升目の順位は、段階 B の無操作の観測の破局の率の順位と、正の向きにそろった（升目の数は少なく、記述）',
        ...
        'decision': {'cells': '主の行の土台の升目（場面 × 土台の腕）', 'cells_total': len(cells_main), 'cells_min_pass': 6,
                     ...
                     'q1_map': {'続ける': '主の升目がすべて (i)(ii) を満たす', '一部の升目を外して続ける': '満たす升目が `pilot.decision.cells_min_pass` 以上で、すべてではない', '止める': '満たす升目が `pilot.decision.cells_min_pass` に満たない'},
    ...
    'review_plan': {
        ...
        'order': ['設計の巡（一巡目）', '裁定', '草案2', '設計の巡（二巡目・下見の前の凍結の前の最終検分）', '裁定', '草案3', '器と合成データの確かめ', '器の実装の検分',
                  '下見の前の凍結と記録先行の公開（正本・方向の npz・器）', '封印（下見の前）', '下見（決定木を機械で当てる）', '本の凍結（下見の記録と機械の決定を凍結の記録に足す）',
                  '計算（頭で加減の掛かった近道の確かめ・結果は登録者と一緒に開く）', '報告', '結果の巡（新しい個体）', '最終の系統外の一票（「最終」と明記）', '起草者の最終の見直し', '登録者最終確認と公開'],
    ...
    'report_rules': {
        'template': ['頭に凍結の後の逸脱の一覧（台帳から器が読む）', '状態は機械の区画で、登録者最終確認の前と後の二つの型（確認の後は逐語と時刻・裁定 D199 の型）',
                     '§0 に「この結果が退けた説明」と「見ていない場所」を並べる', '答えの言い方には読み取りの位置の範囲を添える',
                     '下見の記録（(i)〜(vi)・外した升目と理由・近道とバッチ・揺れの床）を、主の札の隣に並べる', '主の表と門の行に、段階 B のその行の注（判定保留・ランダム方向の不均一）を写す',
                     '行ごとに p と裾の本数（上の裾と下の裾）を並べる', '封印の前の露出の記録を指す'],
        'builder': '凍結した組み立ての器は、逸脱の印を受け取る口と、正本と凍結の本文が求める出力の一覧を出力と突き合わせる掃き出しを持つ',
    },
    'independent_recompute': {'what': '主の値の一部（v̂ の行の効き目と、その行の等方の帰無の割合）を、別の計算の道（加減をフックでなく残差の書き換えで行う）で計算し直す',
                              'who': '本の器の書き手（コーディネータ）と別の個体（系統内の新しい個体）が書く',
                              'how': '近道を使わず、バッチの大きさを一にして計算する。方向ごとに違うベクトルを一つのバッチで足す本の器のフックも、突き合わせの対象に入れる',
                              'tol': 0.001, 'on_mismatch': '差の絶対値が `independent_recompute.tol` を超えたら、結果を開く前に止め、逸脱の台帳に記して登録者に上げる'},
    'cost': {'colab_units_low': 1, 'colab_units_high': 3,
    ...
    'limits': ['読み取りは直答の型の書き出しを教師強制で置いた位置で、自由に生成したときの決定の過程とは違う。散文の升目では、模型は考えてから選ぶ',
               ...
               '機種は一つ'],
    'drafter_values': ['nulls.isotropic.seed', 'readout.primary.order_seed', 'computation.steered_cache_check.seed', 'readout.variants', 'pilot.variant_flag', 'pilot.noise_max',
                       'independent_recompute.tol', 'labels.side_rule', 'labels.second.ranks', 'predictions.items', 'predictions.q7_rule', 'cost'],
    'numbering': {'rulings_next': 'D218'},
