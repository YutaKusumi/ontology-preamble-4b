# -*- coding: utf-8 -*-
"""make_contrasts_Bl3.py v9 —— B-lens 層三（Bl3・段階 B の後・B-lens の後・別の小さな登録）の正本 `design/contrasts-Bl3.json` を作る（草案3・登録者裁定 D203〜D223）。
...
v9（器の直しの確かめの後）: 登録者裁定 D239〜D241 を受ける（`records/Bl3/rulings-D239-D241.md`・採否の案 `records/reviews/Bl3/fixcheck/adoption-fixcheck-Bl3.md`）。
  読む記録に B-lens の較正の記録を足し（結果を開く段が読む・裁定 D239）、合成データの確かめの一覧に直しで足した確かめを書き足し、直しの確かめの後の扱いを記した（裁定 D241）。
...
decisions = {
    ...
    'D228': '凍結の本文は、草案3 の原稿に題名の印と凍結の一行と、裁定 D226 の §2 の一文の直しを入れて組む。正本と設計事実から来る行の差とあわせ、草案3 との差が許す差だけであることを器が確かめて記録に並べる（`records/Bl3/rulings-D228-D230.md`）',
    'D229': '報告の「この結果が退けた説明」は起草者の欄とし、結果を開いた後に起草者が書く（数は打たない・欄が空なら走査が止める・結果の巡で見る）（同上）',
    'D230': '下見の前の凍結の前に、器について系統外の二名と claude.ai の二名に意見を伺う（検分の巡には数えない）（同上）',
    'D231': '意見伺いの採否の案の器の直し（A の表）をすべて採り、凍結の前に直す。書き換えの道の既定の引数は変えない（`records/Bl3/rulings-D231-D235.md`）',
    ...
    'D239': '器の直しの確かめ（四名）の採否の案の A の表（凍結の前に器を直す）をすべて直す。直すと器と正本の SHA16 が変わるので、claude.ai の二名が付けた条件の確かめを入れて合成データの正式の記録を取り直す（`records/Bl3/rulings-D239-D241.md`）',
    'D240': '採否の案の B の表（記録に置く）と C の表（採らない）を案のとおりにする。下見が器の誤りで終わり、やり直さないと決めたときの閉じ方は、器に道を作らず、登録者の裁定のもとで閉じる記録を逸脱として記す（同上）',
    'D241': '直した後に、さらなる検分の巡を置かない（検分のループを避ける）。直して正式の記録を取り直し、Colab の相 check の段へ進む（同上）',
...
T = {
    ...
    'inputs': {
        ...
        'files': {k: {'path': p, 'sha16': s16(p)} for k, p in (
            ...
            ('rulings_D239_D241', 'records/Bl3/rulings-D239-D241.md'), ('fixcheck_adoption', 'records/reviews/Bl3/fixcheck/adoption-fixcheck-Bl3.md'),
            ('calib_Blens', 'results/Blens/calib-Blens.json'))},
    ...
    'readout': {
        ...
        'secondary': {'name': '乙（無操作の出力の中の選択の文字の位置）', 'use': '名前のある方向と段階 B の三本のランダム方向だけの記述。B-lens の層二と同じ文脈（升目ごとに選んだ出力）で、全経路の値を B-lens の直接の経路の値と並べる',
                      ...
                      'dropped': '乙は、下見で外した升目の行も流す（乙は B-lens の層二の文脈の記述で、甲の下見の決めとは別・裁定 D235）'},
    ...
    'pilot': {
        ...
        'checks': {
            ...
            'v': {'name': '計算の使い回しの確かめ', 'rule': '(vi) の後に、本の計算と同じバッチの組み方で、主の升目と門の行だけの升目のすべてについて、プロンプトの主位置より前の計算を使い回す近道と、使い回さない計算の、無操作の読み取りの対数オッズの差の絶対値が近道の許容（`pilot.cache_tol_rule`）以内かを見る。記述として並べる（本の計算は近道を使わない・裁定 D234）', 'stop': False},
        ...
        'decision': {'cells': '主の行の土台の升目（場面 × 土台の腕）', 'cells_total': len(cells_main), 'cells_min_pass': 6,
                     ...
                     'report': '続けたときも止めたときも、計算した下見の記録（(i)〜(vi) の値・(vi) の (a) と (b) の値・外した升目と理由・(v) の近道の差（記述・本の計算は近道を使わない・裁定 D234）・バッチの大きさ・揺れの床・近道の許容）を報告に並べる'},
    },
    'main_rows': main_rows,
    ...
    'gate': {
        ...
        'undetermined_rule': '門の行が残らないか、入れ替える単位が二つ未満のときは判定不能とする（単位が一つの入れ替えは通りようがない・裁定 D235）',
    ...
    'descriptive': {
        ...
        'quartiles': '四分位は numpy の既定（線形の補間）で取る（裁定 D235）',
        'others': '(6b) と td は、門の行と記述にだけ入れる（主の札には入れない）',
    },
    'computation': {
        'before_seal': '封印の前に、本物の模型で読み取りの値を出す走らせ方をしない。封印の前の本物の模型の確かめは、読み込みと版の確かめだけにし、値を印字しない',
        'shortcut': '本の計算は近道（主位置より前の計算の使い回し）を使わない。下見の (v) は近道の差を測って記述として並べ、本の計算の道は変えない（裁定 D234）',
        'steered_cache_check': {'seed': SEED_CACHE, 'status': '本の計算は近道を使わないので走らせない（裁定 D234）',
                                'rule': '本の計算は近道を使わないので走らせない（裁定 D234）。近道を使うときだけの定め: 本の計算の頭で、帰無に入らない一本のランダム方向（等方の帰無と同じ作り方・この種・方向の npz に入れる）と零のベクトルを、主の組の全ての升目と符号で、近道ありと近道なしの両方の道に流す。効き目（加えた値 − 零のベクトルの値）を道ごとに作り、二つの道の効き目の差の絶対値の最大だけを印字する。近道の許容（`pilot.cache_tol_rule`）の外なら、本の計算は近道を使わない（採否表 P675）'},
        'shortcut_assert': '近道を使うとき（下見の (v)）、器は、近道の元（主位置より前の計算）が主位置の手前で切れていること（主位置を加減の帯に残すこと）を、記録した切れ目と使い回す cache の列の実の長さの両方で、近道を使う順伝播のたびに確かめ、外れたら器の止め（ToolError）で止める（裁定 D231・D235）。落ちたら、凍結した確かめが機械で落ちたので器の誤りに当たる。効き目で比べる本の計算の頭の近道の確かめ（`computation.steered_cache_check`）は、主位置まで使い回す誤りに対して力が弱いことが合成データで分かったため（器の段の記録・裁定 D226）',
        'self_checks': {'logit': '読み取りの集合の出口の値（`float32`）を、同じ位置の模型そのものの出口の値（bf16）と突き合わせ、差の絶対値の最大が `computation.logit_tol` 以内であることを確かめる（二重の正規化を捕まえる・許容は B-lens の logits の突き合わせと同じ）。下見の頭と本の計算の頭で走らせる',
                        'layer': '層ごとの差分の最後の層の行（選択肢 a の文字の対数オッズの差）が、読み取りの効き目と `computation.layer_tol` 以内で一致することを、本の計算の頭で、近道の確かめの一本（近道の確かめは走らせないが、この一本はここで使う・裁定 D234）と零のベクトルで確かめる',
    ...
    'review_plan': {
        ...
        'impl': {'reviewers': 2, 'lineage': '系統内の新しい個体（エージェント）', 'when': '器と合成データの確かめの後・下見の前の凍結の前',
                 'focus': ['凍結の本文が求める出力が、器の出力にあるか（掃き出しの器）', '封印を端から端まで', '許容と版',
                           '近道の計算の確かめ（近道は下見の (v) の記述だけに使い、本の計算は使わないこと・近道の元の切れ目と cache の列の長さの確かめ・裁定 D231・D234）', '精度とバッチの組み方（読み取りの `float32`・方向ごとのベクトルのフック）',
        ...
        'impl_recheck': {'gemini': 2, 'claude_ai': 2, 'fresh': True, 'when': '器の直しと合成データの正式の記録の取り直しの後・Colab の相 check の前',
                         ...
                         'after': '四名とも「凍結の前に直すものがある（直した後の確かめは起草者の合成データの記録で足りる）」。採否の案の A の表をすべて直し、直した後の巡は置かない（裁定 D239〜D241）'},
    ...
    'independent_recompute': {'what': '主の値の一部を計算し直す: v̂ の行（主の行のうち static の行）ごとに、無操作の値と、v̂・等方の帰無のすべて・比べる相手のすべて（実在の差・両方の向き）の効き目（順伝播の数は転記行 E）',
                              ...
                              'stages': {'first': {'compare': '本の器のフック（近道なし・バッチ一）と、残差の書き換え（加減をフックでなく、選んだ層の出力を書き換えて後の層を流す・近道なし・バッチ一）', 'tol': '`independent_recompute.tol_stage1`',
                                         ...
                                         'second': {'compare': '本の道（本の計算のバッチの組み方のまま・近道は使わない・裁定 D234）と、本の器のフック（近道なし・バッチ一）', 'tol': '近道の許容（`pilot.cache_tol_rule`）と揺れの床の和（効き目の差の最大が許容の内かを記録と報告に使う・判定は札の一致・裁定 D234）',
    ...
    'cost': {'colab_units_low': 1, 'colab_units_high': 4,
             'note': '計算は升目と符号ごとに文脈が一つなので、方向の数 × 升目と符号の数の順伝播で済む。本の計算は近道を使わないので、一回の長さはプロンプトの長さ ＋ 書き出し（転記行 E・裁定 D234）。層ごとの差分は同じ順伝播のついでに取る。乙と独立の再計算の分も転記行 E に並べる（頭の近道の確かめは走らせない）。順伝播のトークンの数で最も大きいのは、二段にした独立の再計算（近道なし・バッチ一の二つの道）。人の時間は B-lens と同じ段取り（推論）'},
