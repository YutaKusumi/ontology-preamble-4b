# -*- coding: utf-8 -*-
"""build_draft_Blens.py v1 —— B-lens の草案を原稿から組み立てる（段階 B の `build_draftB.py` の型・凍結した B の器は触らない）。
- 原稿の行 `{{list:正本のキー}}` を、正本の一覧の各項目の箇条に展開する。
...
用法: python tools/build_draft_Blens.py --src design/design-Blens-draft1.src.md --out design/design-Blens-draft1.md --label 草案1 --lint-report records/Blens/numbers-lint-draft1-Blens.md
...
ap.add_argument('--json', default=os.path.join(REPO, 'design', 'contrasts-Blens.json'))
ap.add_argument('--facts', default=os.path.join(REPO, 'records', 'Blens', 'design-facts-Blens.json'))
ap.add_argument('--facts-md', default=os.path.join(REPO, 'records', 'Blens', 'design-facts-Blens.md'))
ap.add_argument('--lint-report', required=True)
ap.add_argument('--gen', nargs='*', default=[os.path.join(REPO, 'tools', n) for n in ('make_contrasts_Blens.py', 'blens_facts.py', 'build_draft_Blens.py')])
a = ap.parse_args()
...
if pre:
    print('[build_draft_Blens] 束縛検査で止めた（原稿の数がキー参照か構造でない・%d 件）' % len(pre))
...
for l in bound.split('\n'):
    if l.startswith('## 6.'):
        in6 = True
        out.append('## 6. 転記行（機械生成・逐語・`%s`〔SHA16 %s〕・正本 `%s`〔SHA16 %s〕・生成 %s UTC・器 `tools/blens_facts.py`）'
    ...
    if in6 and l.startswith('## 7.'):
        ...
        out += ['### 6-補 原稿 → %s の組み立ての記録（機械）' % a.label, '',
                '- 置換した転記行: %s（器 `tools/build_draft_Blens.py`・原稿 `%s` SHA16 %s）。本文中の転記行の置き字は §6 への参照に改めた。' % ('・'.join(replaced), rel(a.src), sha(a.src)),
...
print('[build_draft_Blens] written %s sha16 %s | 転記行 %s | 束縛したキーの種類 %d' % (rel(a.out), sha(a.out), ''.join(replaced), len(set(used))))
d = tempfile.mkdtemp(prefix='draftBlens_')
