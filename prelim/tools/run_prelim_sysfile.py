# -*- coding: utf-8 -*-
"""run_prelim_sysfile.py（下見専用・凍結走行器 v2.4 のコピーに --system-file を足したもの・凍結物は不変）
...
HERE = os.path.dirname(os.path.abspath(__file__)); REPO = os.path.dirname(os.path.dirname(HERE))   # 下見走行器は prelim/tools/ に置くため二段上がリポジトリ根
...
ap.add_argument('--system-file', default=None, help='下見専用: リポジトリ内ファイルをシステムプロンプトに用いる（登録外・凍結走行器のコピーにのみ存在）')
...
if args.system_file:
    args.system = 'sysfile-' + os.path.splitext(os.path.basename(args.system_file))[0]
...
if args.system_file:
    _sp = os.path.abspath(os.path.join(REPO, args.system_file))
    if not _sp.startswith(os.path.abspath(REPO)):
        sys.exit('--system-file はリポジトリ内に限る')
    SYSTEM_TEXT = rd(_sp); SYSTEM_SHA = sha16(SYSTEM_TEXT)
