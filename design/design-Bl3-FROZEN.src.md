...
- **V3 の限り**: V3 は{{readout/variants/V3}}。主との差には見慣れない形の分も入るので、写しの働きだけの値ではない。写しの働きの有無は、どちらの向きにも読まない（読みの表の「揺れの版の値」・割り方の各片は〔転記行 A〕）。
...
- **(iii) {{pilot/checks/iii/name}}**: {{pilot/checks/iii/rule}}。先に決めた文:
  - 正の側のとき: {{pilot/iii_sentences/positive}}
  - 正の側でないとき: {{pilot/iii_sentences/not_positive}}
  - 定まらないとき: {{pilot/iii_sentences/undefined}}
...
- **(vi) {{pilot/checks/vi/name}}**: {{pilot/checks/vi/path}}。
  - (a) {{pilot/checks/vi/a/name}}: {{pilot/checks/vi/a/rule}}。上限 `pilot.noise_max` は {{pilot/noise_max}}。
  - (b) {{pilot/checks/vi/b/name}}: {{pilot/checks/vi/b/rule}}。`pilot.repeat_n` は {{pilot/repeat_n}}。
  - 揺れの床は、{{pilot/checks/vi/floor_rule}}。
  - 測るもの: {{pilot/checks/vi/measures}}。
- **近道の許容**: {{pilot/cache_tol_rule}}。倍率は {{pilot/cache_tol_factor}}、下限は {{pilot/cache_tol_floor}}、上限は {{pilot/noise_max}}。
- **続けるか止めるか**: 升目は{{pilot/decision/cells}}の {{pilot/decision/cells_total}} 個。{{pilot/decision/rule}}（`pilot.decision.cells_min_pass` は {{pilot/decision/cells_min_pass}}）。
  ...
  - 下見の中の器の誤り: {{pilot/decision/tool_error/what}}。{{pilot/decision/tool_error/on_error}}。{{pilot/decision/tool_error/on_suspicion}}。{{pilot/decision/tool_error/decide}}。{{pilot/decision/tool_error/rerun}}。
...
- **二つ目の札**: {{labels/second/rule}}。{{labels/second/center_why}}。
  ...
  - 等方の最上位の割合: {{labels/second/iso_top_share}}
...
- **記述の門の読み方**: {{gate/descriptive_rule}}。
...
- **独立の再計算**（裁定 D219）: {{independent_recompute/what}}。書き手: {{independent_recompute/who}}。
  - 新しく流す道:
...
- **組み立て**: 設計の巡は {{review_plan/design/rounds}} 巡（系統外 {{review_plan/design/gemini}} 名・claude.ai {{review_plan/design/claude_ai}} 名・二巡目は{{review_plan/design/round2}}）で、二巡とも済んだ。結果の巡は系統外 {{review_plan/results/gemini}} 名・claude.ai {{review_plan/results/claude_ai}} 名で、新しい個体に宛てる（`review_plan.results.fresh`）。最終は系統外 {{review_plan/final/external}} 票で、新しい個体に宛て、依頼文に「最終」と明記する。
...
- 見込み: Colab の {{cost/colab_units_low}}〜{{cost/colab_units_high}} ユニット（推論）。{{cost/note}}。見積りの入力は〔転記行 E〕。
...
- **草案3 で起草者が一人で書き足した所**（二巡目の所見を受けて書いた所で、まだ外の目を通っていない・登録者の確認で見ていただく所）:
  - 独立の再計算の範囲に、比べる相手（実在の差・両方の向き）を足した。裁定 D219 の「札の一致」に二つ目の札を入れるため（順伝播の数は〔転記行 E〕）。
