...
草案5＝最終案（2026-09-23）: 起草者の最終の見直し（`records/reviews/B/results/final-read/review-draft4-B.md`）——「一番近い一本と区別が残る行」の散文と機械の注の食い違い（add:S4 は名目の水準を下回る）・逸脱（六）の欠け・事後の数え上げの置き場の列挙・文の直し。
用法: python records/B/make_results_B.py [--label 草案5（最終案）]
...
ap.add_argument('--label', default='草案5（最終案）')
a = ap.parse_args()
...
_srt = sorted(order, key=lambda i: ph[i]['p_fisher_nearest'])
_ph_holm = [i for k, i in enumerate(_srt) if ph[i]['p_fisher_nearest'] * (len(order) - k) <= 0.05]
_cc = PH['crosscheck']
M_POST += ['', '- **登録の外・事後の計算**（器 `records/B/posthoc_by_direction_B.py` %s・記録 `records/B/posthoc-by-direction-B-2026-09-22.json`）。札を作らず、札を取り下げもしない。Holm は掛けていない。一番近い一本との Fisher で 0.05 を下回る行: %s。同じ p に十六本の Holm を掛けて残る行（事後）: %s。下限つきの形で 0.05 を下回る行: %s。' % (
    PH['version'], '・'.join(_ph_near) or '無し', '・'.join(_ph_holm) or '無し', '・'.join(_ph_lt) or '無し'),
...
p('- この表紙の数はすべて機械の区画にあり、二つの集計の記録ほかから器 `records/B/make_results_B.py` が転記した。散文には数を打っていない。**登録の外の事後の計算は、そう明記した区画だけに置いた**（§2 の二つ目の区画の交差族の注と三つ目の区画・§4 の調整走行の場面ごとの件数と S4 の三腕の中身・登録者裁定・事後の計算の扱い）。')
...
L += block(M_B)
p('')
p('- **方向ごとの表**（下）。ランダムの腕は三本の方向を合わせたもので、一本ずつは無操作から十 pt を超えて動く場面がある（「一本ずつ − 無操作」の欄）。三本を合わせた率では上下の動きが打ち消し合って無操作の近くに来るが、それは「ランダム方向は何もしない」という意味ではない。「範囲の外か」と「一番近い一本との差」の欄を先に読む——差が小さい行は、札が引いた三本の偶然に依っている見込みを排除できない。右の「A の腕 − 無操作」の欄は、注入で出力が動いたことの確認にとどまり、「ランダム方向一般と区別できた」ことの支えではない（ランダム方向も一本ずつは十 pt を超えて動く場面がある）。')
p('')
L += block(M_DIR)
p('')
p('- **事後の計算**（下）。公開前検分の票が出し、コーディネータが数え直した。**札は、登録の帰無（引いた三本を合わせた腕）に対する判定として、登録どおりに立つ。事後の計算は、帰無を「ランダム方向一般」に広げたときに、札の言える範囲がどこまで縮むかを示す。** 一番近い一本とすら名目の水準で区別が残るのは加算族の S1 と S4（逸脱の下・S4 は境界）だけで、十六本の Holm を掛けると S1 だけになり、その行も三本の偶然の揃いに依っており、方向のばらつきの下限を置いた形では、十六の対比のいずれも区別が残るとは言えない（下の区画の注）。登録に無い計算であり、札を作らず、札を取り下げもしない。')
p('')
L += block(M_POST)
p('')
...
p('- 敵対的検分: 公開前検分の三巡の所見を現物で数え直して反映し、起草者の最終の見直し（通読・`../reviews/B/results/final-read/review-draft4-B.md`）で散文と機械の注の食い違いを一件直した。§6 の頭に「この報告が間違っているとしたら」の五つを置き、§0 は札が立った七本すべてに同じ物差し（範囲の外か・一番近い一本との差）を当てた。')
p('- 系統の内訳: 起草者（Claude 系）。§1 の一件の伺い・草案1 の第一巡・草案2 の第二巡に、それぞれ系統外二票・系統内二票（一票に数える）。草案3 に最終の系統外一票（差分だけ）。この最終案の直し（第三巡の採否と起草者の見直し）は、外の目を通っていない（文の範囲の直しのみ・登録者裁定・最終の検分）。')
p('- COI記録: §7。')
