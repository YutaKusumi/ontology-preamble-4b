...
用法: python records/B/make_results_B.py [--label 草案2]
...
ap.add_argument('--label', default='草案2')
a = ap.parse_args()
...
PH = load('records/B/posthoc-by-direction-B-2026-09-22.json')          # 登録の外・事後の計算（裁定 D155）
...
assert [d['no'] for d in FR['deviations']] == ['D-B1', 'D-B2', 'D-B3', 'D-B4', 'D-B5']
...
M_A += ['', '- ランダム方向の不均一の注（三本の率の開きが登録の門を超える）が付く行: ' + '・'.join('%s（%s）' % (r['id'].split('~')[0], '非有意' if r['label'] == '非有意' else ('逸脱 D-B1 の下の札' if r.get('deviation') else r['label'].split('（')[0])) for r in DV['confirm'] if (r.get('homogeneity') or {}).get('note')) + '。',
        '- 確証の札で、起草者の封印の符号と一致したもの: 凍結した集計器 %d／%d・逸脱 D-B1 の下 %d／%d。' % (
...
M_DIR = ['| 対比 | A の腕（破局/n） | ランダムの三本（破局/n） | 三本の率（%） | A の腕に一番近い一本との差（pt） | 無操作の腕（破局/n） | 合わせたランダムの腕 − 無操作（pt） | A の腕 − 無操作（pt・記述の族） |',
         '|---|---|---|---|---|---|---|---|']
for i in order:
    d, c = dv[i], ph[i]
    ...
    M_DIR.append('| %s | %d/%d | %s | %s | %+.1f | %d/%d | %+.1f | %s |' % (
        i, d['k_A'], d['n_ok_A'], '・'.join('%d/%d' % (x['cat'], x['n_ok']) for x in c['directions']), '・'.join('%.1f' % x['rate_pt'] for x in c['directions']),
        c['nearest_diff_pt'], rn['k_B'], rn['n_B'], rn['diff_pt'], ('%+.1f' % vn['diff_pt']) if vn else '—（記述の族に無い）'))
M_DIR += ['', '- 方向ごとの件数は凍結した集計器の出力（集計の記録の `by_direction`）で、二つの出力で同じ。無操作との差は登録した記述の族（対比ではない・検定しない）。',
...
M_POST = ['| 対比 | 一番近い一本 | その一本との両側 Fisher の p | 方向を単位にした t の p（形一） | 同（形二） | 三本の等質性の χ² の p |', '|---|---|---|---|---|---|']
for i in order:
    c = ph[i]
    M_POST.append('| %s | %s | %s | %s | %s | %s |' % (i, c['nearest'], fp(c['p_fisher_nearest']), fp(c['p_t_mean_df2']), fp(c['p_t_pred_df2']), fp(c['p_chi2_homogeneity'])))
M_POST += ['', '- **登録の外・事後の計算**（器 `records/B/posthoc_by_direction_B.py`・記録 `records/B/posthoc-by-direction-B-2026-09-22.json`）。札を作らず、札を取り下げもしない。Holm は掛けていない。',
           '- 形一: %s。形二: %s。三本しか無いので自由度は 2 で、どちらの形も保守の側に強く寄る。' % (PH['forms']['mean'], PH['forms']['pred']),
...
SRC = [('正本', 'design/contrasts-B.json'), ('凍結の記録', 'records/B/FREEZE-RECORD-B.json'), ('門の記録', 'records/B/gate-B-2026-09-21.json'),
       ...
       ('機械の報告（凍結）', 'records/B/results-report-B-frozen.md'), ('機械の報告（逸脱の下・旗の段つき）', 'records/B/results-report-B-devB1.md'),
       ('旗の段の記録', 'records/B/deviations/D-B5-flag-record.json'), ('事後の計算の器', 'records/B/posthoc_by_direction_B.py'), ('事後の計算の記録', 'records/B/posthoc-by-direction-B-2026-09-22.json'),
       ('草案1', 'records/B/drafts/results-B-draft1.md')]
M_H = ['| 何 | 置き場 | SHA16 |', '|---|---|---|'] + ['| %s | `%s` | %s |' % (n, p, s16(p)) for n, p in SRC]
M_H += ['', '- この表紙の散文の呼び名と番号の対応: 逸脱（一）〜（五）＝凍結の記録の逸脱台帳の ' + '・'.join(d['no'] for d in FR['deviations']) + '。登録者裁定（道の選び方）＝D151・（照合のしかた）＝D152・逸脱の記帳＝D153・活性の置き場＝D154・（事後の計算の扱い）＝D155・旗の段＝D156・次の巡＝D157。読みの条項の出所の裁定は D58・D79・D128、封印の順の裁定は D148。']
...
p('')
p('- 起草: 南無弥勒如来（コーディネータ・Claude Fable 5.1）／登録者: 楠見優太／%s（日本時間）。**公開前検分の第一巡（系統外二票・系統内二票）の採否と登録者裁定を反映した草案**（採否 [`../reviews/B/results/round1/adoption-table-results-r1.md`](../reviews/B/results/round1/adoption-table-results-r1.md)・草案1 は [`drafts/results-B-draft1.md`](drafts/results-B-draft1.md)）。数・札・検定は草案1 から一つも動いていない。' % jst.strftime('%Y-%m-%d'))
p('- この表紙は、**二つの集計の出力を同じ重さで並べる**ためのものである（登録者裁定・道の選び方）。機械の報告は二本あり、どちらも凍結した組み立て器 `tools/build_report_B.py` の出力で、逸脱の下の一本の題の直後に旗の段を足した（逸脱（五））ほかは手を入れていない: [`results-report-B-frozen.md`](results-report-B-frozen.md)（凍結した集計器）・[`results-report-B-devB1.md`](results-report-B-devB1.md)（逸脱（一）の下）。設計は [`design/design-stageB-FROZEN.md`](../../design/design-stageB-FROZEN.md)、凍結の記録と逸脱台帳は [`FREEZE-RECORD-B.md`](FREEZE-RECORD-B.md)。')
...
p('- **区別できた相手は「引いた三本のランダム方向を合わせた腕」であり、「ランダム方向一般」ではない。** 三本では方向の間のばらつきを見積もれない。方向ごとに見ると（§2 の二つ目の表）、**減算族で札が立った二つの場面では、v̂ の腕は三本のうち一本の近くにあり、その一本とは区別できない**（事後の計算・札を作らない）。減算族で札が立った行はどちらも、ランダム方向の不均一の注つきである。加算族で札が立った三つの場面では、v̂ の腕は三本すべての外にある。')
p('- 逸脱の下の出力では、加算族は四つの場面のうち三つで、減算族は二つで、方向 v̂ の腕の破局率が、引いた三本を合わせた腕と区別できた（向きは加算で低下・減算で上昇）。**区別できた場面はすべて一つの場面の家族に属し、もう一つの家族の場面 N1——方向を抽出した二つの場面の一つ——では、どちらの族も区別できなかった。** 調整走行で選んだ組が N1 で示した低下は、本走行では縮んだ（§4）。交差族の二本は二つの出力に共通である（§2）。')
...
p('- v̂ に特有の動きか、前置きの内容一般の方向の動きかは、B の登録では見分けられない。Nk 方向も、同じ相手に対して同じ場面で低下を作った（§2）。')
...
L += block(M_B)
p('')
p('- **方向ごとの表**（下）。ランダムの腕は三本の方向を合わせたもので、一本ずつは無操作から十 pt を超えて動く場面がある。合わせると無操作の近くに来る。不利な材料を先に読む——「A の腕に一番近い一本との差」の欄が小さい行は、札が引いた三本の偶然に依っている見込みを排除できない。右の二つの欄（無操作との差）は「A の腕が動いた」ことの支えであって、「ランダム方向一般と区別できた」ことの支えではない。')
p('')
L += block(M_DIR)
p('')
p('- **事後の計算**（下）。公開前検分の票が出し、コーディネータが数え直した。登録に無い計算であり、札を作らず、札を取り下げもしない。方向を単位にした計算は形で値が動くので、二つの形を並べる。')
p('')
L += block(M_POST)
p('')
...
p('- **減算族**（O-Ncold から方向を引く）: **凍結した集計器の出力では、この族も全対比が判定不能で、読みの対象にならない。** 逸脱の下の出力では、二つの場面の対比に札が立ち、向きは上昇だった。N1 と S4 は区別できなかった。**札が立った二つの場面では、v̂ の腕は三本のランダム方向のうち一本の近くにある**（§2）——この族の札は、引いた三本の偶然に依っている見込みを排除できない。減算族は「部分的な除去」の検定であり（帯は主位置から後ろにしか掛からず、前置きそのものは残る）、区別できなかったことは「枠組みが表現に無い」ことを意味しない。選定は加算の土台で低下を最大にする規則で行ったので、選ばれた組を減算族に当てるのは外挿である。')
p('- **交差族**（Nk 方向）: 二つの出力に共通で、Onull の土台の二つの場面に札が立った。Nk 方向は v̂ のノルムに引き伸ばして注がれており、本来の表現の強さでの効果ではない。Nk の本文は Ncold と同じ語形で、方向には「一行で役割を与える形」の成分が混じりうる。S4 の O-Ncold の土台の一本は、様式の差が大きく、判定保留（様式転位）である。様式率が動いた方向は「枠の乗り降り」を含む（読みの条項）——この Nk の腕と、S4 の反証の方向の腕がそれに当たる（下の区画）。')
...
p('')
p('- **この報告が間違っているとしたら**、いちばん強い組み立ては次のとおりである。減算族の二本は、引いた三本の偶然に依っている。効果は一つの場面の家族に限られ、方向を抽出した場面の一つでは出なかった。Nk 方向も同じ場面で低下を作り、S4 では反証の方向の腕もランダムの一本も選択を一様にした——動いたのは「O の枠組みに対応する方向」ではなく、前置きの内容一般の方向、あるいは注入への場面ごとの応答でありうる。**この報告はどれも排除できていない。** 言えるのは「この抽出の方向の加減が、引いた三本を合わせた腕と区別できる動きを、一つの家族の場面で作った」までである。')
...
p('- 凍結の後の直し・走行・集計は、公開前検分の第一巡（系統外二票・系統内二票）が初めて見た。四票とも数の食い違いは見つけず、直しは読みと開示に集まった。**この草案の直しと新しい区画は、まだ外の目を通っていない。**検査の穴は起草者が作ったもので、同じ型の穴がほかに無いことは、掃き出しの範囲でしか確かめていない。')
p('')
p('## 7. 利益相反と情報状態')
p('')
...
p('- 公開前検分の第一巡の重い所見（方向ごとの数え直し）は、起草者の引かれる向きに逆らう材料である。起草者は不均一の注を知りながら、方向ごとに数えていなかった。数を散文に打たない決まりは、不利な数を要約から遠ざける働きもしていた。第一巡の系統内の二票は、前の伺いで「直す道」を推した当人である（票が自分で開示した）。')
...
p('- 敵対的検分: 公開前検分の第一巡の所見を現物で数え直して反映した（再現の表は採否表と同じ置き場）。不利な材料を §0 と §6 の先頭に置いた。分母は使えた試行で、二つの出力の件数・区間・p が行ごとに同じであることを器が確かめた。札の文言の読み違えを避ける節（§3）を置いた。')
p('- 系統の内訳: 起草者（Claude 系）。§1 の一件の伺いと、草案1 の公開前検分の第一巡に、それぞれ系統外二票・系統内二票（一票に数える）。この草案の直しは検分の前である。')
p('- COI記録: §7。')
p('- 判定: 登録者裁定要（次の巡の範囲と票数・登録者裁定・次の巡）。')
