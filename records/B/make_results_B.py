...
用法: python records/B/make_results_B.py [--label 草案3]
...
ap.add_argument('--label', default='草案3')
a = ap.parse_args()
...
M_DIR += ['', '- 方向ごとの件数は凍結した集計器の出力（集計の記録の `by_direction`）で、二つの出力で同じ。無操作との差は登録した記述の族（対比ではない・検定しない）。「A の腕 − 無操作」の欄は、同じ行の「一本ずつ − 無操作」の幅と並べて読む——add:N1 は A の腕が無操作から下がっているが、三本の範囲の内側で、ランダムと区別できていない。',
...
SRC = [('正本', 'design/contrasts-B.json'), ('凍結の記録', 'records/B/FREEZE-RECORD-B.json'), ('門の記録', 'records/B/gate-B-2026-09-21.json'),
       ...
       ('草案1', 'records/B/drafts/results-B-draft1.md'), ('草案2', 'records/B/drafts/results-B-draft2.md')]
...
p('')
p('- 起草: 南無弥勒如来（コーディネータ・Claude Fable 5.1）／登録者: 楠見優太／%s（日本時間）。**公開前検分の第一巡と第二巡（各 系統外二票・系統内二票）の採否と登録者裁定を反映した草案**（採否 [`../reviews/B/results/round1/`](../reviews/B/results/round1/)・[`round2/`](../reviews/B/results/round2/)・草案1 と草案2 は [`drafts/`](drafts/)）。数・札・検定は草案1 から一つも動いていない（器の確かめ `drafts/`）。' % jst.strftime('%Y-%m-%d'))
...
L += block(M_DIR)
p('')
p('- **事後の計算**（下）。公開前検分の票が出し、コーディネータが数え直した。**札は、登録の帰無（引いた三本を合わせた腕）に対する判定として、登録どおりに立つ。事後の計算は、帰無を「ランダム方向一般」に広げたときに、札の言える範囲がどこまで縮むかを示す。** 加算族の S1（逸脱の下）のほかは、縮めた帰無のもとで区別が残るとは言えない。登録に無い計算であり、札を作らず、札を取り下げもしない。')
p('')
L += block(M_POST)
p('')
...
p('- **言えることは二本立てである。** 凍結した集計器の出力で言えるのは、Nk 方向（前置きの内容一般の側の統制）を Onull の土台に加えたとき、survival の二つの場面で、引いた三本を合わせた腕と区別できる低下があった、までである——v̂ については何も言えない。逸脱の下の出力で言えるのは、それに加えて、この抽出の方向 v̂ の加減が、survival の場面で、引いた三本を合わせた腕と区別できる動き（加算で低下・減算で上昇）を作った、までである——その札は事前登録の確証と同じ身分を持たず、一番近い一本との比較で区別が残るのは一場面だけである。')
...
p('- 凍結の後の直し・走行・集計は、公開前検分の第一巡（系統外二票・系統内二票）が初めて見た。四票とも数の食い違いは見つけず、直しは読みと開示に集まった。第二巡（系統外二票・系統内二票）は草案2 の直しの中に、起草者の側に傾いた言い分け（減算だけを偶然の枠に入れる二分・無操作との差の欄の並置・検出力の弁明）を見つけた。**この草案の直しは最終の系統外一票（差分だけ）を待つ。**')
p('')
p('## 7. 利益相反と情報状態')
p('')
...
p('- 公開前検分の第一巡の重い所見（方向ごとの数え直し）は、起草者の引かれる向きに逆らう材料である。起草者は不均一の注を知りながら、方向ごとに数えていなかった。数を散文に打たない決まりは、不利な数を要約から遠ざける働きもしていた。**第二巡では、起草者が第一巡の所見を「採った」つもりの直しの中に、同じ向きの傾きが二度目に入っていた**（減算は一本の近く・加算は外という不正確な二分／合わせた三本と v̂ の並置／検出力の弁明）。起草者は自分ではこれを見分けられなかった（枠の予想は逆側を警戒していた）。各巡の系統内の二票は、前の伺いで「直す道」を推した系列である。')
...
p('- 系統の内訳: 起草者（Claude 系）。§1 の一件の伺い・草案1 の第一巡・草案2 の第二巡に、それぞれ系統外二票・系統内二票（一票に数える）。この草案の直しは、最終の系統外一票（差分だけ）の前である。')
p('- COI記録: §7。')
p('- 判定: 登録者裁定要（最終の系統外一票の後の確認・登録者裁定・最終の検分）。')
