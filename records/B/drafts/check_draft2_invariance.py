# -*- coding: utf-8 -*-
"""草案1 と草案2 の表紙を突き合わせる: 草案1 の機械の区画の表の行（対照表・札の文言の表・td の表・予想の照合・S4 の区画・門と走行の区画）が、
...
diff = list(difflib.unified_diff(d1, d2, 'results-B-draft1.md', 'results-B.md（草案2）', lineterm='', n=1))
out = os.path.join(HERE, 'diff-draft1-draft2.diff')
open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(diff) + NL)
rec = dict(kind='check_draft2_invariance', draft1_sha16=s16(NL.join(d1)), draft2_sha16=s16(NL.join(d2)), machine_lines_draft1=len(m1), machine_lines_draft2=len(m2),
...
json.dump(rec, open(os.path.join(HERE, 'check-draft2-invariance.json'), 'w', encoding='utf-8', newline=NL), ensure_ascii=False, indent=1)
