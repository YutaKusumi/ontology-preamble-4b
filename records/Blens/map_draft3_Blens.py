...
MAP2 = {
    ...
    555: ('§7 限界', [('draft', '様式の感度に残る「私は」は、O の腕（と N1 の O-Ncold）の出力の書き出し')]),
...
REWRITE = {   # 第一巡の行の文字列 → 草案3 での文字列（書き換えた第二巡の採否の行）
    ...
    (522, '写した器が、凍結の関数の再生（本走行と調整走行の種・三本）とビットで一致する'): ('写した器が、凍結の関数の再生（本走行と調整走行の種で、層ごとに引く全ての方向）とビットで一致する', '二度目の通読'),
    (524, '**情報状態**: 予想者（登録者とコーディネータ）は、封印の前に設計の巡の四票'): ('**情報状態**: 予想者（登録者とコーディネータ）は、封印の前に、設計の巡の二巡の八票', 'P551・二度目の通読'),
}
used_rw = set()
out2, out1, bad = [], [], []
