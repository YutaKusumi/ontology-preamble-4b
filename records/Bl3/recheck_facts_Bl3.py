# -*- coding: utf-8 -*-
"""recheck_facts_Bl3.py v1 —— B-lens 層三（Bl3）の設計の事実（`records/Bl3/design-facts-Bl3.json`）の件数を、器 `tools/bl3_facts.py` のコードを使わずに数え直す（読み取りだけ）。
数え直すもの: JSON 直答の型の出力の総数と升目ごとの内訳・その選択・散文の出力の件数と書き出しを含む件数・主の升目の JSON 直答の件数・使えた試行の件数・量が零の (a) の件数（無操作の升目と門の行）。
...
L = ['# B-lens 層三の設計の事実の数え直し（機械生成・`records/Bl3/recheck_facts_Bl3.py` v1）', '',
