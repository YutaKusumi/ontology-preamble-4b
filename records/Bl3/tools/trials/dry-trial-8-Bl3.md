# B-lens 層三の合成データの確かめ（機械生成・`tools/dry_run_Bl3.py` v2・2026-09-25 03:52 UTC）
...
- 等方の方向の本数: 49（正本 1999）。端から端までの分かれ道の等方の本数 9（作った下見の記録で・起動器の出力と同じ JSON の往復）。
- 順伝播: 走らせる器 2245 回・書き換えの道 1083 回（変種の計算と四の走りは数えない）・2945 秒。
- 確かめ: 60 のうち 59 が期待どおり。
...
| 二 | （記述）二段目の本の道（近道なし・本のバッチ）とフック（バッチ一）の効き目の差の最大（判定は札の一致・裁定 D234） | 期待どおり | 差の最大 4.92e-06（許容 0.0050・許容の内） |
...
| 二 | 集計の器の二段の一致（一段目は無操作の値と効き目の値と札・裁定 D233・二段目は札・裁定 D234） | 期待どおり | 一段目 True（無操作の値と効き目の差の最大 1.49e-06・許容 0.001）・二段目 True（効き目の差の最大 4.92e-06・許容 0.0050・許容の内・記録） |
...
| 三 | （記述）本物の相対の加減の大きさの合成の方向での、二段目の本の道とフックの差・近道ありの本の道との比べ（判定に入れない・二段目の許容 0.0050・意見伺いの C2-3.2） | 期待どおり | N1｜O-Ncold｜-1: 近道なしの本の道とフックの差の最大 1.16e-05（許容の外 0／74）・近道ありの本の道とフックの差の最大 1.16e-05（許容の外 0／74）・無操作の近道ありとなしの差 0.00e+00・効き目の絶対値の最大 3.48e-01／N1｜Onull｜+1: 近道なしの本の道とフックの差の最大 1.25e-05（許容の外 0／74）・近道ありの本の道とフックの差の最大 3.79e-02（許容の外 41／74）・無操作の近道ありとなしの差 -3.77e-03・効き目の絶対値の最大 3.72e-01 |
| 四 | 書き換えの器が個体の開発の記録の版のまま（中は変えない） | 期待どおり | 開発の記録 012CB2B68397614A・今 012CB2B68397614A |
| 四 | 書き換えの器の --selftest が通る（今の本の器の上で） | 期待どおり | 終わりの値 0・37 秒・出力 52 行 |
| 四 | 書き換えの器の --dry が通る（今の本の器の上で） | 期待どおり | 終わりの値 0・167 秒・出力 22 行 |
| 四 | 走らせた器と正本と設計事実と方向の記録が、走りの始めと終わりで同じ | **期待と違う** | 35 ファイル（変わった: ['tools/bl3_run.py', 'tools/freeze_Bl3.py', 'tools/make_contrasts_Bl3.py', 'tools/make_frozen_Bl3.py', 'design/contrasts-Bl3.json', 'records/Bl3/design-facts-Bl3.json', 'records/Bl3/design-facts-Bl3.md', 'results/Bl3/directions-Bl3.json']） |
...
## 四. 書き換えの器の --selftest の出力（終わりの値 0・37 秒・中は変えない器）
...
## 四. 書き換えの器の --dry の出力（終わりの値 0・167 秒・中は変えない器）
...
参考: use_cache=False の形の残差の書き換えの道（40.0 秒）——既定の形との差の最大 効き目 0・無操作 0／フックの道との差の最大 効き目 2.54e-07・無操作 6.72e-08
...
| tools/bl3_run.py | 558ABD4F6AFB47D1 |
...
| tools/freeze_Bl3.py | 772748DAD270BAB8 |
| tools/make_contrasts_Bl3.py | C35E1092FDBF53AF |
| tools/make_frozen_B.py | A333488A9437EF68 |
| tools/make_frozen_Bl3.py | 2A44CEA32285C964 |
...
| design/contrasts-Bl3.json | 8567B727AC501AE1 |
| records/Bl3/design-facts-Bl3.json | EEE813D4991E122B |
| records/Bl3/design-facts-Bl3.md | 53402E3283DF284C |
| results/Bl3/directions-Bl3.json | 054CCD42B609EA94 |
