# B-lens 層三の合成データの確かめ（機械生成・`tools/dry_run_Bl3.py` v4・2026-09-26 00:04 UTC）
...
- 順伝播: 走らせる器 2035 回・書き換えの道 1003 回（変種の計算と四・五・六の走りは数えない）・4681 秒（五の起動器の三つの相 1040 秒・等方 9 本・六の DRY でない枝 287 秒）。
- 確かめ: 87 のうち 87 が期待どおり。
...
| 一 | 門の行の割り当ての照らし（鍵を転記行 C と・家族の鍵を鍵の決まりと・行動の量を段階 B の集計の記録から正本の文で・裁定 D239） | 期待どおり | 行 64・行動の量の食い違い 0・家族の鍵の食い違い 0・転記行 C との食い違い 0 |
| 一 | 取り違えの見本（鍵を重ねない三つ）: 答えの分かる合成は一のまま通り、割り当ての照らしが捕まえる（裁定 D239） | 期待どおり | 家族の名の入れ替え: 答えの分かる合成 一のまま・割り当ての照らし 捕まえた・行動の量の入れ替え: 答えの分かる合成 一のまま・割り当ての照らし 捕まえた・単位の入れ替え: 答えの分かる合成 一のまま・割り当ての照らし 捕まえた |
| 一 | 台帳の器の差分を路ごとにつなげる照らし（二度の直しは通り・切れと無い変えと現れない差分は止まる・裁定 D239） | 期待どおり | 差分なし・同じ 通る・台帳に無い変え 止まる・台帳に記した変え 通る・同じ置き場の二度の直し 通る・前後の切れ 止まる・記したのに現れない 止まる |
| 一 | 書き換えの道の有限でない値は判定の段で不一致として記す（例外で落ちない・等方 1999 本・裁定 D239） | 期待どおり | NaN の無い二つの道 一致 True／書き換えの道の v̂ の効き目の一つを NaN に → 不一致 True・理由 non_finite |
...
| 二 | 本の計算は近道を使わない（振る舞い: 近道の元を作る呼び出し・使い回す cache・use_cache・列の全長を全ての順伝播で数えた・裁定 D236） | 期待どおり | 順伝播 47 回・近道の元を作った回 0・cache を渡した回 0・use_cache が偽でない回 0・列の全長でない回 0 |
...
| 四 | 書き換えの器の --selftest が通る（今の本の器の上で） | 期待どおり | 終わりの値 0・58 秒・出力 52 行 |
| 四 | 書き換えの器の --dry が通る（今の本の器の上で） | 期待どおり | 終わりの値 0・306 秒・出力 22 行 |
| 五 | 起動器の三つの相（DRY・別のプロセス）: 相 check は順伝播を呼ばずに終わり、呼ばれた数と升目のトークンの並びの SHA16 を書く | 期待どおり | 終わりの値 0・順伝播を呼んだ数 0・守り 7・升目 9 |
| 五 | 起動器の三つの相（DRY・別のプロセス）: 相 check の中の守りの掛かった呼び出しを Exception で呑もうとしても、守りが止めて止めの zip を作る（裁定 D239） | 期待どおり | 終わりの値 1・止めの zip 1・印字 ['[boot_Bl3] 止める（相 check の守り・登録者に相談）: 相 check で順伝播が呼ばれた（model.model・正本 computation.before_seal）'] |
...
| 五 | 起動器の三つの相（DRY・別のプロセス）: 組の出力の中身を変えて session はそのままなら、一致だけを見る段が止まって書かない（組の SHA-256・裁定 D239） | 期待どおり | 組 main の出力の SHA-256 が、起動器が session に書いた値と違う（止める）: 今 2486214330832B0B…・session DD81AE1AFF94A865… |
| 五 | 起動器の三つの相（DRY・別のプロセス）: 一致だけを見る段の後に組の出力と session をそろえて変えると、結果を開く段が止まって書かない（裁定 D236） | 期待どおり | 結果を開く段の出力が、一致だけを見る段の読んだ出力と違う（開かない）: ['main', 'recompute', 'secondary'] |
| 六 | DRY でない枝: 凍結の本文の器を一時の複製で端から端まで走らせ、組み立ての記録の行が凍結版の原稿を指し、手元の道筋が無く、組み直しが同じ（裁定 D239） | 期待どおり | 組み立ての終わりの値 0・組み直しの確かめの終わりの値 0・記録の行が凍結版の原稿を指す True・手元の道筋 0 |
| 六 | DRY でない枝: 予想の書式を組み、下見の前の凍結の確かめ（Colab の確かめを除く）で凍結の本文と書式と正本と方向と器の自己検査が通る（外れは、取り直す前の合成データの正式の記録のものだけ） | 期待どおり | 書式の終わりの値 0・確かめの外れ 2（合成データの正式の記録の外の外れ 0） |
| 六 | DRY でない枝: 本の凍結の器が、下見の試みのコミット・封印の記録の錨・凍結物の照らしを通って本の凍結を記す（台帳の行の数を記す・裁定 D239） | 期待どおり | 終わりの値 0・[freeze_Bl3] 本の凍結を記帳した: records/Bl3/FREEZE-RECORD-Bl3.json（下見の試み 1） |
| 六 | DRY でない枝: 一致だけを見る段（等方は正本の本数・凍結の記録と手元の器・相 main が使った下見の記録・session のコミットの本の凍結）→ 台帳の追記 → 結果を開く段 → 掃き出し → 報告の器の CLI（走査の違反 0）が通る（裁定 D239） | 期待どおり | 終わりの値 判定 0・開く 0・掃き出し 0・報告 0・wrote records\Bl3\results-Bl3.md（走査の違反 0） |
| 六 | DRY でない枝: 組の JSON だけを書き換えると一致だけを見る段が、組と session をそろえて書き換えると結果を開く段が、判定の記録を書き換えると結果を開く段が、止まって書かない（裁定 D236・D239） | 期待どおり | 組 main の出力の SHA-256 が、起動器が session に書いた値と違う（止める）: 今 320C7CE57986F9C5…・session B2FC96BD86264481…／結果を開く段の出力が、一致だけを見る段の読んだ出力と違う（開かない）: ['main']／一致だけを見る段を同じ入力でもう一度走らせた答えが、判定の記録と違う（開かない）: ['env']（もう一度の理由 None） |
| 六 | DRY でない枝: 一致だけを見る段の後に、凍結の記録の台帳の外を書き換えても、判定の時にあった台帳の行を書き換えても、結果を開く段が止まる（台帳は後ろに足すだけ・裁定 D239） | 期待どおり | 凍結の記録（台帳の外）が一致だけを見る段の後に変わった（開かない）／凍結の記録の台帳の、一致だけを見る段の時の行が変わった（開かない・台帳は後ろに足すだけ） |
| 六 | DRY でない枝: 器を変えると、台帳に無ければ一致だけを見る段と起動器の版の照らしが止まり、台帳に記せば通り、同じ器を二度直しても通り、台帳に無い三度目の変えで止まる（裁定 D239） | 期待どおり | 一致だけを見る段の終わりの値 1・0・0・1／起動器の版の照らしの外れ 1・0・0・1 |
| 六 | DRY でない枝: 相 main が使った下見の記録が手元の本の凍結の下見と違うと、また session のコミットの凍結の記録の本の凍結が手元と違うと、一致だけを見る段が止まる（裁定 D239） | 期待どおり | [analyze_Bl3] 一致だけを見る段: 器の誤り 無し・組の環境 同じ・一段目 無い・二段目（札） 無い・二段目の値 無い・全体 無い（値は開いていない）・理由 組 main の相 main が使った下見の記録が、手元の本の凍結の下見の記録と違う: {'batch': 1／[analyze_Bl3] 一致だけを見る段: 器の誤り 無し・組の環境 同じ・一段目 無い・二段目（札） 無い・二段目の値 無い・全体 無い（値は開いていない）・理由 相 main のコミット b9f9f01f6d49 の凍結の記録の本の凍結が、手元の本の凍結と違う（または読め |
| 六 | DRY でない枝: 報告の器の CLI は、DRY の集計・一致だけを見る段の記録の欠け・封印の記録の書き換え（公開したコミットの錨）で止まり、戻すと通る（裁定 D239） | 期待どおり | DRY の集計の記録で本の報告は組まない（止める）／一致だけを見る段の記録が無い（報告を組まない）: records/Bl3/judge-Bl3.json／報告を組む前の確かめが外れた（止める・登録者に相談）: ['封印の記録が、本の凍結の記録にある下見の試みのコミット（b9f9f01f6d49）の封印の記録と違う（または読めない）', '封印の記録の SHA16 が、本の凍結の記録に写した値と違う']／戻すと終わりの値 0 |
...
## 四. 書き換えの器の --selftest の出力（終わりの値 0・58 秒・中は変えない器）
...
## 四. 書き換えの器の --dry の出力（終わりの値 0・306 秒・中は変えない器）
...
参考: use_cache=False の形の残差の書き換えの道（78.6 秒）——既定の形との差の最大 効き目 0・無操作 0／フックの道との差の最大 効き目 2.54e-07・無操作 6.72e-08
