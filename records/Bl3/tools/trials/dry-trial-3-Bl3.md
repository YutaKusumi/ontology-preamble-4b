# B-lens 層三の合成データの確かめ（機械生成・`tools/dry_run_Bl3.py` v1・2026-09-24 21:11 UTC）
...
- 等方の方向の本数: 49（正本 1999）。順伝播 421 回・393.2 秒。
- 確かめ: 33 のうち 33 が期待どおり。
...
| 二 | 本の計算: 全ての方向と無操作がそろい、埋めた零のベクトルの値は使わない | 期待どおり | 升目と符号 17 |
| 二 | 零のベクトルの行は無操作と同じ値になる（別のバッチでも） | 期待どおり | 効き目 0.00e+00（揺れの床 4.81e-06） |
| 二 | バッチの中の位置で方向を取り違えない | 期待どおり | 位置を入れ替えた効き目の差の最大 9.47e-07 |
| 二 | 近道ありと近道なしの効き目が許容の内で合う | 期待どおり | 差の最大 0.00e+00（許容 0.0050） |
| 二 | 層ごとの差分（名前のある方向と段階 B の三本・等方の中央値と中央の区間） | 期待どおり | 層 2・等方 49 本 |
| 二 | 二段目（本の道とフック・近道なし・バッチ一）の効き目の差が「近道の許容＋揺れの床」の内 | 期待どおり | 差の最大 4.65e-06（許容 0.0050） |
...
| 二 | 集計の器の二段目の一致（合成・一段目は独立の再計算の器ができた後） | 期待どおり | 二段目 True（差の最大 4.65e-06・許容 0.0050）・一段目 まだ無い・全体の一致 False |
