# B-lens 層三の凍結の本文と草案3 の差の記録（機械生成・`tools/make_frozen_Bl3.py` v4）
...
+- 束縛: 原稿のキー参照を正本 `design/contrasts-Bl3.json`（SHA16 33E543663FE37A5F）の値で置換した。組み立ての前に束縛検査、後に登録検査と生成器の文字列リテラル検査を走らせた（記録 `（組み直しの一時の置き場の数の検査の記録）`）。
...
- 組み直しと凍結の本文の差の残り（(一) と (三) の外）: 無し
- 凍結版の原稿を同じ手順で組み直した本文と凍結の本文: 同じ
