...
| ファイル | 断片 | 件数 | 推定トークン数 |
|---|---|---|---|
| `records/A/judge-trial/judge-files/judge-file-A-1of1.md` | F0001〜F0613 | 613 | 599,170 |
...
## 3. 一つの添付ファイルを一人の判定者に読んでもらう手順（判定者 6 名 × ファイル 1 本＝会話 6 回）
...
4. 返信を最初から最後まで順に写し、返信を貼るファイル `records/A/judge-trial/judge-replies/<判定者>-<ファイルの番号>.txt`（例: `Gemini1-1.txt`）に貼って保存します。空のファイルは用意してあります。
