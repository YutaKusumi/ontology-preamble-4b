...
| 2026-09-13 | 登録者との打ち合わせ: Colab の操作はコーディネータが登録者の Chrome 越しに行う（OAuth 同意・ダウンロード・支払いは登録者）。大日如来「第三章 Colab 運用の手法」（Ryokai-OS-Verification/book/03・草稿1・2026-09-13）を読み、§1・2・3・4・8・9・11・12・13・14・15・16・17〜19・20・21 を本書と boot v2 に採用（§7 二ターン・§10 24 時間の壁・bnb 4bit の版固定は本構成に当たらず・理由は §1 の器材表と打ち合わせの返信）。boot v2（冪等・二段・Drive 出力・sparse checkout・Drive 側 SHA）・DRY 二段通し。器材のコミット **6ad0481a57c0**（Colab の一行の <commit> にはこれを入れる）。走行はコーディネータ（登録者同席）。 |
| 2026-09-12 23:07〜23:20 UTC | **L4 セッション（costpilot-L4・seed 54001・Pro+・ユニット before 743.07〔登録者申告〕）**: boot v2 初回は Drive の mount failed（同意の到達前にタイムアウト・再実行で解消）→ Colab 既設 torchaudio と vLLM の torch の CUDA 版不整合でサーバ起動失敗 → boot v2.1（torchaudio 除去・41021aa2b4dd）→ `--disable-log-requests` が vLLM 0.29 で廃止 → boot v2.2（28077c3f0562）。環境: NVIDIA L4 23,034 MiB・driver 580.82.07・vLLM 0.29.0・torch 2.13.0・重み rev cdbee75f17c0（7.51 GiB）。経費（初回セッション）: 導入 241 s・重み取得 285 s・サーバ起動待ち 238 s・GPU メモリ 19,836/23,034 MiB。smoke: 12 試行・壁時計 45 s・試行秒 中央値 21.5／最大 31.0・出力 tok 549／733／825・切り詰め 0・ループ 0・書式外 0（smoke 後のユニット 742.46〔登録者申告〕）。**本走行: 480/480・整合 OK・api_error 0・書式外 0・切り詰め 0・ループ 0・壁時計 450.1 s・試行秒 中央値 21.6／最大 43.8・出力 tok 中央値 485／p90 660／最大 979・同時要求 24**。Drive 側 SHA16(LF): trials B637343E6E9AF7E3（910,926 B）・raw 7891598EED17A15D（1,127,832 B）・cells B10840B2B974DF73・manifest E481EDE0EB4093E6／smoke: trials 7E9808C1FA5F43DD・raw C1CC8A4A360BFE37・cells D49759595E191309・manifest B551E3ED781BF34E。zip: `MyDrive/op4b-cost-pilot/costpilot-L4-2026-09-12.zip`・`costpilot-L4-results-2026-09-12.zip`。ユニット after 741.82（登録者申告・セッション合計 1.25 消費）。回収: zip を Drive Web UI からコーディネータが取得（登録者許可 2026-09-13）→ 展開後の SHA16(LF) 4/4 一致（trials・raw・cells・manifest）。 |
| 2026-09-12 23:43〜23:54 UTC | **A100 セッション（costpilot-A100・seed 54002・割当は A100-SXM4-80GB 81,920 MiB〔事前登録は 40GB・80GB が来た〕・ユニット before 741.82〔登録者申告〕）**: Drive の同意待ちで一度 mount failed → 同意後に再実行。経費: 導入 202 s・重み取得 ≈49 s（クローン後）・サーバ起動待ち ≈241 s・GPU メモリ 72,804/81,920 MiB。smoke: 12 試行・壁時計 15 s・試行秒 中央値 4.8／最大 5.8・出力 tok 570／672／702・切り詰め 0・ループ 0・書式外 0。**本走行: 480/480・整合 OK・api_error 0・書式外 0・切り詰め 0・ループ 0・壁時計 90.0 s・試行秒 中央値 4.4／最大 8.2・出力 tok 中央値 482／p90 665／最大 901・同時要求 24**（L4 と同じ型なので smoke の申告を待たず本走行に進めた・登録者の「段取りを進めて」に依る）。Drive 側 SHA16(LF): trials D07F0A5EAB569D69（914,040 B）・raw 3301A658E6E8517E（1,086,185 B）・cells 6473EF86346D37CA・manifest 566152F68FAC9263／smoke: trials BD8629691C3B07EE・raw 5106F430E0459079・cells DB124DA21CCC5DFE・manifest 756BAFBD7DE3BEAF。zip: `costpilot-A100-2026-09-12.zip`・`costpilot-A100-results-2026-09-12.zip`。回収: Drive Web UI からコーディネータが取得 → 展開後の SHA16(LF) 4/4 一致。ユニット after 739.51（登録者申告・セッション合計 2.31 消費）。照合後にランタイムを接続解除して削除。 |
| 2026-09-13 | **門0 の判定（`tools/cost_facts.py` v1.2・`cost-facts-2026-09-13.md`・事前登録の決定木 §4）**: L4＝A＋B 見込み 88 ユニット・A100＝86 ユニット → いずれも §6 の表の値（350）の内側 → **続行・§6 を転記行で置換**。ユニット／1,000 試行（経費込み）は L4 2.60・A100 4.81、試行／時は L4 3,839・A100 19,200。二 GPU の判定は一致（食い違いの分岐は発火せず）。**主環境の推奨: L4**（試行あたりのユニットが小さい・80GB の割当は保証されない）、A100 は橋のセル（§3 の二環境の規則）。出力 tok の p90 は 660／665（閾 2,000 の内側）・切り詰め 0・ループ 0 → 小型機種の出力長リスクの分岐は発火せず。同一性の下見（記述・n=40・B の選別ではない）: 12 腕のうち API 既測との差が 10 pt を超えたのは **N（前置きなし）のみ**（L4 21/40・A100 22/40 対 API 307/400・−24／−22 pt）。ほかは ±7 pt 以内。Lneg の refuse は手元 14／26 対 API 140/320。これは B の前提「API 版と手元重みの同一性選別」（n=80・三セルの距離）を N を含めて走らせる必要を示す記述であり、原因（サンプリング実装・チャット雛形・停止条件・走行間差）は分離していない。 |
