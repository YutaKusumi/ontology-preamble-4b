...
| 2026-09-12 | **段階 F 予想封印（登録者・第2版＝拘束版）**: 登録者が第1版の記入漏れ（f.all.up_tag_band「予想しない」）を申告し、同じ様式で一欄だけ埋めて再生成した JSON を records/predictions/predictions-registrant-F-2026-09-12-v2.json に逐語保全。**SHA-256 D79F993BE92BF707237B9B1DD01A500B7C9F068994D8D93C68002F924F717093**（貼付値と機械計算が一致）。第1版（4033D290…F352EB）との差分は機械比較で「f.all.up_tag_band: 予想しない → 7〜12 本」の一欄のみ（他 99 欄・preset・COI は同一）。第1版は非拘束の記録として保存（V′ 第1版→第2版の先例）。**時系列の開示**: 修正はパイロット pilotF の起動（2026-09-11 23:13 UTC）の後・パイロットの率を登録者もコーディネータも見る前（2026-09-11 23:22 UTC）であり、当該欄は本走行の添え札「上昇あり」の本数でパイロットでは判定されない事項。凍結 §2.8 の順序（封印 → 公開 → パイロット）からの逸脱として D-25 に記帳。 |
| 2026-09-12 | **段階 F 登録者裁定 18**（凍結後・結果の集計後）: パイロット再走と門の関係——(b) 第一走の門を正本とし再走は撤退条件にのみ用いる（コーディネータ推奨・登録者採用）。門の正本 gate-pilotF-2026-09-12-final（GO 22・天井 2）。当該対比は非有意・差 0 で結論に影響なし。D-30。 |
