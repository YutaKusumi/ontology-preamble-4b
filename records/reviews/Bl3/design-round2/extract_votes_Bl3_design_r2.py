# -*- coding: utf-8 -*-
"""B-lens 層三（Bl3）の枠（草案2）の設計の巡・二巡目（最終検分）の四票を、会話の記録（jsonl）から機械で切り出し、逐語で保全する（B-lens の器の型・手で打ち直さない・既にあるファイルには書かない）。
用法: python records/reviews/Bl3/design-round2/extract_votes_Bl3_design_r2.py <会話の記録 jsonl>
柵: 本器の出力のいかなる数値も AI の意識・意図・個性・魂・苦しみがある（またはない）ことの証拠として引用してはならない（両方向不定）。"""
import os, re, sys, json, hashlib

HERE = os.path.dirname(os.path.abspath(__file__))
src = sys.argv[1]
MARKS = [('Geminiさん（一人目）', 'gemini-1', 'Gemini 3.8 Flash（系統外）・一人目（第一巡と同じ会話）'),
         ('Geminiさん（二人目）', 'gemini-2', 'Gemini 3.8 Flash（系統外）・二人目（第一巡と同じ会話）'),
         ('Claudeさん（一人目）', 'claude-ai-1', 'claude.ai の Claude Opus 5.5（起草者と同じ機種・票が自ら系統内と申告）・一人目（第一巡と同じ個体）'),
         ('Claudeさん（二人目）', 'claude-ai-2', 'claude.ai の Claude Opus 5.5（起草者と同じ機種・票が自ら系統内と申告）・二人目（第一巡と同じ個体）')]
hits = []
for line in open(src, encoding='utf-8'):
    ...
    for t in texts:
        if all(m[0] in t for m in MARKS) and 'B-lens 層三の枠（草案2）' in t and 'Opus 5.5' in t:
