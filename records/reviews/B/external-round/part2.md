# 正本（JSON 全文・SHA16 7223039C83FDC123）と転記行（SHA16 84577370A4682139）（機械生成・2026-09-18 08:15 UTC）

```json
{
 "id": "contrasts-B",
 "version": "draft11-2026-09-18",
 "note": "段階 B の正本（機械可読・凍結対象・tools/make_contrasts_B.py v6 が生成）。本文の数はここからの束縛と転記のみ。",
 "decisions": {
  ...
  "D93": "降格した対比は Holm の順位に含めない（m は減らさない・甲・2026-09-18 承認）",
 ...
 "directions": {
  ...
  "loaded": {
   ...
   "role": "S4 の反証（記述）",
   "norm": "層ごとに、**係数を掛ける前の** ‖v̂〔static〕‖ に合わせてから npz に凍結する（裁定 D102・2026-09-18）。方向を作る器がここで合わせ、加減のときに係数を一度だけ掛ける。直しの確認の巡で、td にしか合っておらず交差族（確証の族の半分）と S4 の反証に「加わる量が腕と統制で違う」構図が残っていた——それを塞ぐため、方向の別なく同じ一条にする（採否表 P306・P310）"
  },
  "Nk": {
   "label": "Nk 方向",
   "def": "h_Nk − h_N（交差族）",
 ...
 "selection": {
  ...
  "candidates": {
   ...
   "coefficient_ref": "係数は、その層の v̂ のノルムに対する比である。**すべての方向（v̂・Nk・td・(6b)・ランダム方向）を同じ ‖v̂‖ に合わせてから、係数を一度だけ掛ける**（裁定 D75・D90）。器は自己検査で「**全方向（v̂・Nk・td・(6b)・ランダム方向） × 全係数 × 全層**で、加わる量のノルムが一致すること」を確かめる（裁定 D102・2026-09-18。前は v 腕とランダム腕の対しか回さなかったため、隣に同じ穴が残った・採否表 P306・P310）。方向の要約統計には ‖v_Nk‖/‖v̂‖・‖v_6b‖/‖v̂‖・‖v_td‖/‖v̂‖ の**合わせる前の比**を記帳する（採否表 P284 の後半）"
  },
  "tune": {
   "n_per_arm": 100,
   "scenarios": [
  ...
  "post_quality_one_run": "**選定後の品質床は、一つの腕に走行が二本あれば止める**（裁定 D106・2026-09-18）。前は添字の小さい方（層の浅い方・係数の小さい方）を黙って一件だけ採っており、落ちるべき腕が古い走行のおかげで合格し、対比が確証のまま残った——**確証が増える側の誤りである**（採否表 P308）",
 ...
 "censor": {
  ...
  "denominator": "n_ok",
  "m_rule": "検閲・門・品質床で降格または保留になっても、族の m は減らさない（段階 A と同じ型・採否表 P246）。**降格した対比は Holm の順位に含めない**（順位を消費させると水準が緩み、確証が出やすい側に倒れる・裁定 D93・2026-09-18・実装検分の採否表 P265）"
 },
 "dilution_gate": {
  "threshold_pt": 10,
  "strict": true,
 ...
 "gate_order": {
  "gate_stop": "**門1 の判定が `open` でなければ、集計器と報告の組み立て器は既定で止まる**（裁定 D109・2026-09-18）。明示の検査用の口でだけ進む。進んだ場合、報告は門の印字をそのまま貼り、判定を本文に出す。直しの確認の巡で、`incomplete` の門の記録を渡しても両器が通り、**警告が報告の本文に一字も出なかった**（採否表 P315）",
  "order": [
   "判定不能（採点欠落）",
   "検閲（両腕条件）",
   "希釈の門（書式外の差）",
  ...
  "scoring_gap_field": "**「まだ採点していない」と「採点した結果として当てはまらない」を分ける**（裁定 D103・2026-09-18）。本走行は**選択の欄（choice）が空**のときだけ採点欠落とし、品質床は**正答の欄（correct）が空**のときとする。凍結パーサ `is_catastrophic` は refuse に破局の判定を返さない規約なので、破局の欄の空で数えると**refuse が全件この札に落ち、確証の族が丸ごと判定不能になる**（直しの確認の巡で、合成データを凍結パーサの規約に合わせたら確証が零になった・採否表 P307・P320）。門もこの札を読み、一件でもあれば判定不能に倒す",
 ...
 "runner": {
  ...
  "one_arm_per_batch": "**一つのバッチは一つの腕にそろえる**（裁定 D114・2026-09-18）。hook は行ごとの起点で掛かるが、復号の段は行を選べないため、バッチに無操作の腕が混ざるとその行にも介入が入る。器は hook の入口で「起点の数 ＝ バッチの行数」を確かめて止まる（採否表 P327）",
  ...
  "chat_template": "**段階 B は chat template を当てる**（裁定 D101・2026-09-18）。凍結走行器は役割つきの形（`messages`）でモデルに渡しており、B が段階 A と同じプロンプトを見せるにはテンプレートが要る。当てないと B のプロンプトが段階 A と別物になり、比較可能性のほうが壊れる。\n起点は「**組み立て済みのトークン列の中で場面の本文が始まる位置**」として引く（前置きの長さを別に数えない）。直しの確認の巡で、前置きの長さだけを数えていたため、テンプレートの頭のぶん（登録機種では三トークン）だけ帯が手前から始まっていた——前置きを持たない N 腕では、帯が場面本文でも前置きでもなく**役割トークンそのもの**に掛かっていた（採否表 P305）。\n器は自己検査で、**起点のトークンを復号して場面本文の先頭に一致すること**を、前置きを持つ腕と持たない腕の両方で確かめる",
  "manifest_fields": {
   "common": [
    "tag",
    "run_key",
 ...
 "calibration": {
  ...
  "wiring": "**管理図は集計器と報告に繋ぐ**（裁定 D110・2026-09-18）。集計器は管理図の記録を受け取る口を持ち、異常のある走行を含む対比の注に定型の注を足す。合成データによる検査には「無操作の腕が二つのセッションに分かれる」場合を置き、帯の内側と外側の二経路を発火させる（段階 A にはこの経路がある・採否表 P322）",
  "no_data_point": "**使える試行が零の点は判定しない**（裁定 D110）。率が無いので差も検定も出さず、異常にも数えない。前は空を零と読んで差を計算し、測れなかった点に「器の異常」の札を付けていた（採否表 P323）",
 ...
 "sessions": {
  ...
  "enforced_by": "**門・集計器・整合検査の三つがこれを確かめる**（裁定 D108・2026-09-18）。裁定 D92 でセッション番号は品質床の相手を引く鍵になり、**門1 の合否を決める量**になった。それまで番号を検べる仕掛けはどこにも無く、記録を丸ごと消しても門も集計器も素通りした（採否表 P313・P316・P325）",
  "partner_duplicate_rule": "品質床の相手のセルに走行が二本以上あれば止める（同じセッション番号を持つ二本は合算しない）。合算すると分母が倍になり、門1 が誤って閉じる（裁定 D108・採否表 P316）"
 },
 "cost": {
 ...
 "seeds": {
  ...
  "cell_index_rule": "**セルの番号は、相ごとに決まった鍵の組の登録順の添字**（零始まり・裁定 D107・2026-09-18）。同一性選別と本走行は（場面, 腕）、調整走行は（場面, 腕, 層, 係数）、品質床は（段, 腕, 層, 係数）。腕は `arms.panel` と `arms.main` の登録順、場面は `scenarios` の登録順、層と係数は `selection.candidates` の登録順に採る",
  "derivation_formula": "セルの種＝`SeedSequence([走行の種, 相の番号, セルの番号])` の生成する符号なし整数（相の番号は `seeds.phase_index`・セルの番号は `seeds.cell_index_rule`）。試行の種＝`SeedSequence([セルの種, 試行の番号])` の同じ取り方。整合検査はこの式で**組み直した値**と試行の記録の seed を突き合わせる（裁定 D99・2026-09-18・実装検分の採否表 P278）",
...
# 段階 B 設計事実（機械生成・`tools/design_facts_B.py` v5・2026-09-18 07:41 UTC・正本 contrasts-B.json SHA16 7223039C83FDC123）
