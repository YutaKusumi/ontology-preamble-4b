# -*- coding: utf-8 -*-
"""設計の巡・第二巡（凍結前の最終検分・裁定 D178）の四票を会話の記録（jsonl）から機械で切り出し、逐語で保全する（手で打ち直さない・既にあるファイルには書かない）。
第一巡の器 `records/reviews/Blens/design-round1/extract_votes_Blens_design.py` を写し、発言を見分ける語だけを改めた。
用法: python extract_votes_Blens_design_r2.py <会話の記録 jsonl>"""
...
for line in open(src, encoding='utf-8'):
    ...
    for t in texts:
        if all(m[0] in t for m in MARKS) and 'B-lens の枠（草案2）' in t and '凍結前の最終検分' in t and 'Opus 5.5' in t:
...
for i, (mark, d, who) in enumerate(MARKS):
    ...
    if os.path.exists(out):                                   # 逐語保全は上書きしない。既にあれば中身が同じことだけを確かめる
        ...
        print('%-12s 既にある・同一を確認' % d); continue
